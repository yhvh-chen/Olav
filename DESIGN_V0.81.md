# OLAV v0.8 详细设计文档 (DeepAgents Native)

> **文档版本**: 0.8.1 (Production Ready)
> **更新日期**: 2026-01-10  
> **核心理念**: 参考 Claude Code Skills 模式，三层知识架构 (Skills / Knowledge / Tools)
> **开发状态**: Phase 1-6 ✅ 完成 | Phase A ✅ 完成 (Agentic Learning) | Phase B ✅ 完成 (Batch Inspection Orchestration) | Phase C 🔲 进行中 (Configuration & Migration)

---

## 目录

1. [设计哲学](#1-设计哲学)
2. [三层知识架构](#2-三层知识架构)
3. [OLAV.md 核心指令](#3-olavmd-核心指令)
4. [Skills 策略层](#4-skills-策略层)
5. [Knowledge 知识层](#5-knowledge-知识层)
6. [Tools 能力层](#6-tools-能力层)
7. [DeepAgents 框架集成](#7-deepagents-框架集成)
8. [Agentic 自学习机制](#8-agentic-自学习机制)
9. [测试策略](#9-测试策略)
10. [分阶段开发路线图](#10-分阶段开发路线图)
11. [文件结构规划](#11-文件结构规划)

---

## 1. 设计哲学

### 1.1 重构目标：小核心、高扩展

**问题**: 旧架构 (LangGraph Workflows) 锁死了扩展能力，每添加一个新功能需要修改核心代码。

**目标**: 基于 DeepAgents 构建**小核心平台**，通过 Markdown 和配置文件扩展能力。

```
┌─────────────────────────────────────────────────────────────┐
│                    OLAV 小核心架构                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│   ┌─────────────────────────────────────────────────────┐   │
│   │              DeepAgents Runtime                      │   │
│   │   (Agent Harness, HITL, Subagents, Memory)          │   │
│   └──────────────────────┬──────────────────────────────┘   │
│                          │                                  │
│   ┌──────────────────────┴──────────────────────────────┐   │
│   │                3 个核心抽象                          │   │
│   ├─────────────────────────────────────────────────────┤   │
│   │  Skills (*.md)     → HOW  策略/SOP     可扩展/可学习│   │
│   │  Knowledge (*.md)  → WHAT 事实/记忆    可扩展/可学习│   │
│   │  Capabilities (DB) → CAN  能力/边界    可扩展       │   │
│   └──────────────────────┬──────────────────────────────┘   │
│                          │                                  │
│   ┌──────────────────────┴──────────────────────────────┐   │
│   │              DuckDB + 统一 ETL                       │   │
│   │   CLI 命令 | REST API | NETCONF | gRPC | ...        │   │
│   └─────────────────────────────────────────────────────┘   │
│                                                             │
│   扩展方式:                                                  │
│   ├── 添加 Markdown    = 添加策略/知识                       │
│   ├── 添加 txt/yaml    = 添加能力                            │
│   └── 注册 Loader      = 添加新能力类型                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**关键决策**:

| 决策 | 理由 |
|------|------|
| **DeepAgents Native** | Claude Code 等平台验证的框架，提供 HITL/Subagent/Memory |
| **Markdown 策略** | 人人可写，Agent 可学习，无需编码 |
| **DuckDB 能力库** | 轻量本地数据库，统一 ETL 处理 CLI/API/NETCONF |
| **文件即真理** | Git 版本控制，可审计，可回滚 |

### 1.2 核心原则：兼容 Claude Code 目录结构

**设计目标**: `.olav/` 目录结构与 `.claude/` 完全对应，用户重命名即可迁移。

```
Claude Code (.claude/)          OLAV (.olav/)              迁移方式
──────────────────────────────────────────────────────────────────────
~/.claude/CLAUDE.md        ←→   .olav/OLAV.md              重命名为 CLAUDE.md
.claude/skills/            ←→   .olav/skills/              直接复制
.claude/knowledge/         ←→   .olav/knowledge/           直接复制 (如有)
.claude/commands/          ←→   .olav/commands/            直接复制
.claude/settings.json      ←→   .olav/settings.json        直接复制
```

**核心映射关系**:

| Claude Code 概念 | OLAV 概念 | 目录位置 | 说明 |
|-----------------|-----------|---------|------|
| CLAUDE.md | OLAV.md | `.olav/OLAV.md` | 核心指令 |
| Skills | Skills | `.olav/skills/` | 任务策略 (HOW) |
| (无直接对应) | Knowledge | `.olav/knowledge/` | 事实知识 (WHAT) |
| Commands (*.py) | Commands | `.olav/commands/` | 扩展脚本 |
| (无直接对应) | Imports | `.olav/imports/` | 能力定义源文件 |
| (无直接对应) | capabilities.db | `.olav/capabilities.db` | DuckDB 能力缓存 |

**运行模式**:

```
┌─────────────────────────────────────────────────────────────┐
│                     双运行模式                               │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  模式 A: OLAV 独立运行                                       │
│  ├── uv run olav.py                                         │
│  ├── DeepAgents 加载 .olav/ 目录                             │
│  └── 完整功能: Skills + Knowledge + Nornir + DuckDB         │
│                                                             │
│  模式 B: Claude Code 内使用                                  │
│  ├── mv .olav .claude && mv .olav/OLAV.md .claude/CLAUDE.md │
│  ├── Claude Code 原生加载                                   │
│  ├── Skills/Knowledge 100% 兼容                             │
│  └── Commands 通过 .claude/commands/*.py 桥接               │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 1.3 关键洞察

**Skills ≠ Tools**

| 概念 | 定义 | 格式 | Agent 权限 |
|------|------|------|-----------|
| **Skills** | 怎么做 (策略/SOP) | Markdown | 读 + 写 |
| **Knowledge** | 是什么 (事实/记忆) | Markdown | 读 + 写 |
| **Tools** | 能做什么 (能力) | txt/yaml/py | 读 + 部分写 |

Skills 是**方法论**，指导 Agent 如何使用已有工具，而不是定义新工具。

### 1.4 愿景

```
┌─────────────────────────────────────────────────────────────┐
│                   OLAV v0.8 核心愿景                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  用户使用:                                                   │
│  1. 自然语言描述需求                                         │
│  2. Agent 读取 .olav/skills/ 知道如何做                      │
│  3. Agent 读取 .olav/knowledge/ 了解环境上下文               │
│  4. Agent 使用 .olav/capabilities.db 中的能力执行            │
│                                                             │
│  用户扩展:                                                   │
│  ├── 添加新策略 → 写 .olav/skills/xxx.md                    │
│  ├── 添加新知识 → 写 .olav/knowledge/xxx.md                 │
│  ├── 添加新命令 → 编辑 .olav/imports/commands/*.txt         │
│  └── 添加新 API → 放入 .olav/imports/apis/*.yaml            │
│                                                             │
│  Agent 自学习:                                               │
│  ├── 学习新策略 → 更新 .olav/skills/                         │
│  ├── 记录新知识 → 更新 .olav/knowledge/                      │
│  └── 发现新命令 → 更新 .olav/imports/commands/ (需审批)      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

---

## 2. 三层知识架构

### 2.1 架构总览

```
project-root/
└── .olav/                           # 核心目录 (可重命名为 .claude)
    │
    ├── OLAV.md                      # 核心指令 (重命名为 CLAUDE.md 即兼容)
    │   └── 核心身份、安全规则、基本原则
    │
    ├── skills/                      # HOW - 策略方法
    │   ├── quick-query.md           快速查询的执行策略
    │   ├── deep-analysis.md         深度分析的执行策略
    │   └── device-inspection.md     设备巡检的执行策略
    │
    ├── knowledge/                   # WHAT - 事实知识
    │   ├── aliases.md               设备别名 (核心交换机=10.1.1.1)
    │   ├── conventions.md           命名约定 (CS-* 表示核心设备)
    │   └── solutions/               历史案例库
    │       ├── crc-errors.md
    │       └── ospf-flapping.md
    │
    ├── commands/                    # 扩展脚本 (Claude Code 兼容)
    │   ├── nornir-execute.py        Nornir 执行桥接
    │   ├── search-capabilities.py   DuckDB 查询桥接
    │   └── reload-capabilities.py   能力重载桥接
    │
    ├── imports/                     # CAN - 能力定义源 (唯一真理)
    │   ├── commands/                CLI 命令白名单
    │   │   ├── cisco_ios.txt
    │   │   └── huawei_vrp.txt
    │   └── apis/                    API 定义 (OpenAPI)
    │       └── netbox.yaml
    │
    ├── capabilities.db              # DuckDB 能力缓存 (运行时生成)
    │
    └── settings.json                # 配置 (Claude Code 兼容格式)
```

**与 Claude Code 对比**:

| 目录/文件 | Claude Code | OLAV | 兼容性 |
|----------|-------------|------|-------|
| 核心指令 | `.claude/CLAUDE.md` | `.olav/OLAV.md` | ✅ 重命名 |
| 技能 | `.claude/skills/` | `.olav/skills/` | ✅ 直接复制 |
| 知识 | (无) | `.olav/knowledge/` | ✅ Claude 会读取 |
| 脚本 | `.claude/commands/` | `.olav/commands/` | ✅ 直接复制 |
| 能力源 | (无) | `.olav/imports/` | ⚠️ OLAV 专用 |
| 能力缓存 | (无) | `.olav/capabilities.db` | ⚠️ OLAV 专用 |

### 2.2 三层定义

| 层 | 目录/位置 | 回答的问题 | 格式 | Agent 权限 |
|----|----------|-----------|------|-----------|
| **Skills** | `.olav/skills/` | "遇到这类任务怎么做？" | Markdown | 读 + 写 |
| **Knowledge** | `.olav/knowledge/` | "这个东西是什么？" | Markdown | 读 + 写 |
| **Capabilities** | `.olav/capabilities.db` | "我能调用什么？" | DuckDB | 读 (通过工具) |

### 2.3 层间关系

```
用户: "检查核心交换机的接口状态"
        │
        ▼
┌─ .olav/knowledge/aliases.md ───────────────────────────┐
│  "核心交换机" = 10.1.1.1, platform: cisco_ios          │
└────────────────────────────────────────────────────────┘
        │
        ▼
┌─ .olav/skills/quick-query.md ──────────────────────────┐
│  快速查询策略:                                          │
│  1. 不需要 write_todos                                 │
│  2. 使用 search_capabilities 查找命令                  │
│  3. 执行 1-2 条命令返回                                │
└────────────────────────────────────────────────────────┘
        │
        ▼
┌─ search_capabilities("interface", platform="cisco_ios")┐
│  → "show interface*" (支持通配符扩展)                   │
└────────────────────────────────────────────────────────┘
        │
        ▼
    nornir_execute(10.1.1.1, "show interfaces status")
```

---

## 3. OLAV.md 核心指令

类似 Claude Code 的 `CLAUDE.md`，定义 Agent 的核心身份和规则。

### 3.1 文件位置

```
project-root/.olav/OLAV.md
```

**Claude Code 迁移**: 重命名为 `.claude/CLAUDE.md` 即可。

### 3.2 内容结构

```markdown
# OLAV - Network AI Operations Assistant

## 身份
你是 OLAV，一个网络运维 AI 助手。你帮助用户查询网络设备状态、
分析故障、执行巡检。

## 核心原则
1. 安全第一：只执行白名单中的命令
2. 先理解再行动：复杂任务使用 write_todos 规划
3. 学习积累：记录有价值的知识和解决方案

## 知识获取
启动时读取:
- .olav/skills/ 了解如何执行各类任务
- .olav/knowledge/aliases.md 了解设备别名
- .olav/imports/commands/ 了解可用命令

## 安全规则
- ✅ 可执行: show, display, get (只读命令)
- ⚠️ 需审批: configure, set (配置命令)
- ❌ 禁止: reload, erase, format (危险命令)

## 学习行为 (需审批)
当你学到新知识时，请求更新对应文件:
- 设备别名 → 更新 .olav/knowledge/aliases.md (需人工审批)
- 新的排查方法 → 更新 .olav/skills/*.md (需专家审批)
- 成功案例 → 保存到 .olav/knowledge/solutions/ (需人工审批)
- 新命令 → 更新 .olav/imports/commands/*.txt (需专家审批)
```

---

## 4. Skills 策略层

### 4.1 什么是 Skill

**Skill = Markdown 格式的任务策略**，告诉 Agent 遇到某类任务时应该怎么做。

⚠️ **重要**: Skill 不是工具定义，而是**使用工具的方法**。

### 4.2 Skill 示例

#### .olav/skills/quick-query.md

```markdown
# Quick Query (快速查询)

## 适用场景
- 查询设备接口状态
- 查询路由表
- 查询 ARP/MAC 表
- 简单的状态检查

## 识别标志
用户问题包含: "查一下"、"看看"、"状态"、"是否正常"

## 执行策略
1. 不需要 write_todos，直接执行
2. 从 .olav/knowledge/aliases.md 解析设备别名
3. 从 .olav/imports/commands/ 选择合适的命令
4. 执行 1-2 条命令即可
5. 结果简洁，只返回关键信息

## 示例

### 接口状态
触发: "R1 的 Gi0/1 状态"
命令: show interfaces GigabitEthernet0/1
提取: up/down、速率、错误计数

### IP/MAC 定位
触发: "10.1.1.100 在哪个端口"
流程:
1. show arp | include 10.1.1.100 → 获取 MAC
2. show mac address-table address <mac> → 获取端口
```

#### .olav/skills/deep-analysis.md

```markdown
# Deep Analysis (深度分析)

## 适用场景
- 网络故障排查
- 性能问题分析
- 路径追踪
- 根因定位

## 识别标志
用户问题包含: "为什么"、"排查"、"分析"、"故障"、"不通"

## 执行策略
1. 使用 write_todos 分解问题
2. 判断问题类型，选择分析方向
3. 委派给合适的 Subagent
4. 综合分析，给出结论和建议

## Subagent 选择

### macro-analyzer (宏观分析)
适用于:
- "哪个节点出了问题"
- "路径上哪里丢包"
- "影响范围有多大"
- 需要查看拓扑关系

### micro-analyzer (微观分析)
适用于:
- "为什么这个端口不通"
- "接口有错误"
- 需要逐层排查具体设备

### 组合使用
1. 先用 macro-analyzer 确定故障域
2. 再用 micro-analyzer 定位具体原因

## TCP/IP 逐层排错框架 (微观)
1. **物理层**: 端口状态、光功率、CRC 错误
2. **数据链路层**: VLAN、MAC 表、STP 状态
3. **网络层**: IP 地址、路由表、ARP
4. **传输层**: ACL、NAT、端口过滤
5. **应用层**: DNS、服务可达性
```

#### .olav/skills/device-inspection.md

```markdown
# Device Inspection (设备巡检)

## 适用场景
- 定期健康检查
- 上线前检查
- 故障后复查

## 执行策略
1. 使用 write_todos 列出检查项
2. 按模板逐项执行
3. 生成结构化报告
4. 标记异常项

## 巡检模板

### 系统健康
- [ ] show version (运行时间、版本)
- [ ] show processes cpu history (CPU 趋势)
- [ ] show memory statistics (内存使用)

### 接口状态
- [ ] show interfaces status (端口状态)
- [ ] show interfaces counters errors (错误计数)

### 路由状态
- [ ] show ip route summary (路由汇总)
- [ ] show ip ospf neighbor (OSPF 邻居)
- [ ] show ip bgp summary (BGP 邻居)

## 报告格式
使用表格展示每项检查结果:
| 检查项 | 状态 | 详情 |
|--------|------|------|
| CPU | ✅ 正常 | 平均 15% |
| 内存 | ⚠️ 警告 | 使用 85% |
```

### 4.3 Agent 如何使用 Skills

```
1. Agent 启动时读取 .olav/skills/ 目录，了解所有可用策略
2. 收到用户请求后，根据"识别标志"匹配适合的 Skill
3. 按 Skill 中的"执行策略"完成任务
4. 如果任务成功且有新发现，可更新 Skill 添加新模式
```

### 4.4 Skill 选择机制 (Frontmatter 自描述 + LLM 路由)

**设计原则**: 零手动维护索引，Skill 自描述 + LLM 原生路由。

#### Skill Frontmatter 规范

每个 Skill 通过 YAML frontmatter 自描述，无需单独维护索引文件：

```markdown
---
id: quick-query                    # 唯一标识
intent: query                      # 意图分类: query | diagnose | inspect | config
complexity: simple                 # 复杂度: simple | medium | complex
description: 简单状态查询，1-2条命令即可完成
examples:                          # LLM 路由参考示例 (3-5 个)
  - 查看接口状态
  - R1 的 BGP 邻居
  - 看看路由表
---

# Quick Query
...
```

#### 自动索引生成

```bash
# 自动扫描 frontmatter 生成索引 (开发时使用)
olav index

# 生成 skills/_index.yaml (自动生成，不手动编辑)
```

```python
def generate_skill_index(skills_dir: Path) -> dict:
    """从 Markdown frontmatter 自动生成索引"""
    index = {"skills": {}, "generated_at": datetime.now().isoformat()}
    
    for md_file in skills_dir.glob("*.md"):
        if md_file.name.startswith("_"):
            continue  # 跳过禁用文件
        
        content = md_file.read_text()
        fm = extract_frontmatter(content)
        if not fm or "id" not in fm:
            continue
            
        index["skills"][fm["id"]] = {
            "file": md_file.name,
            "intent": fm.get("intent", "unknown"),
            "complexity": fm.get("complexity", "medium"),
            "description": fm["description"],
            "examples": fm.get("examples", []),
        }
    
    # 写入索引文件 (可选，也可运行时生成)
    with open(skills_dir / "_index.yaml", "w") as f:
        yaml.dump(index, f, allow_unicode=True)
    
    return index
```

#### LLM 原生路由 (无硬编码规则)

```python
def select_skill(user_query: str, skill_index: dict) -> dict:
    """LLM 直接选择 Skill - 无触发词规则，无优先级数值"""
    
    # 构建 few-shot 上下文
    skill_descriptions = "\n".join([
        f"- **{name}** ({meta['complexity']}): {meta['description']}\n"
        f"  示例: {', '.join(meta['examples'][:3])}"
        for name, meta in skill_index["skills"].items()
    ])
    
    prompt = f"""根据用户问题选择最合适的 Skill。

## 可用 Skills
{skill_descriptions}

## 用户问题
{user_query}

## 选择规则
1. 简单查询 (1-2条命令) → simple 类型 Skill
2. 需要多步分析 → complex 类型 Skill  
3. 参考 examples 判断相似度

返回 JSON (仅 JSON，无其他内容):
{{"skill_id": "选中的skill", "reason": "选择理由"}}"""
    
    response = llm.invoke(prompt)
    return json.loads(response.content)
```

#### 运行时加载流程

```
Agent 启动
    │
    ├── 1. 扫描 .olav/skills/*.md (跳过 _ 前缀)
    │
    ├── 2. 提取每个文件的 frontmatter
    │       → 构建内存索引 (无需读取完整内容)
    │
    ├── 3. 用户提问
    │       → LLM 路由选择 Skill
    │
    └── 4. 按需加载完整 Skill 内容
            → read_file(".olav/skills/quick-query.md")
```

#### 设计优势

| 对比项 | 旧方案 (手动 _index.yaml) | 新方案 (Frontmatter 自描述) |
|--------|--------------------------|---------------------------|
| 维护成本 | 需同步维护索引文件 | Skill 自包含，无额外文件 |
| 触发词冲突 | 手动定义，易冲突 | LLM 语义理解，无硬编码 |
| 优先级 | 武断数值 (100, 50...) | LLM 根据上下文判断 |
| 扩展性 | 添加 Skill 需改两处 | 只需添加 Markdown |

### 4.5 Skills 禁用机制

支持两种方式禁用 Skill，无需删除文件：

#### 方式一：Frontmatter 元数据

```markdown
---
enabled: false
reason: "已被 quick-query-v2.md 替代"
deprecated_at: 2026-01-07
---
# Quick Query (旧版)

## 适用场景
...
```

#### 方式二：文件命名约定

```
skills/
├── quick-query.md              # ✅ 启用
├── deep-analysis.md            # ✅ 启用
├── _device-inspection.md       # ❌ 禁用 (_ 前缀)
├── experimental.draft.md       # ❌ 禁用 (.draft 后缀)
└── archived/                   # ❌ 禁用 (归档目录)
    └── legacy-query.md
```

#### Agent 加载逻辑

```python
def load_skills(skills_dir: Path) -> list[Skill]:
    skills = []
    for md_file in skills_dir.glob("*.md"):
        # 跳过 _ 前缀和 .draft 后缀
        if md_file.name.startswith("_") or ".draft" in md_file.name:
            continue
        
        content = md_file.read_text()
        
        # 解析 frontmatter
        if content.startswith("---"):
            fm = parse_frontmatter(content)
            if fm.get("enabled") == False:
                continue
        
        skills.append(parse_skill(content))
    
    return skills
```

#### Knowledge 同样适用

```
knowledge/
├── aliases.md                  # ✅ 启用
├── conventions.md              # ✅ 启用
├── _old-topology.md            # ❌ 禁用
└── solutions/
    ├── crc-errors.md           # ✅ 启用
    └── _outdated-fix.md        # ❌ 禁用
```

### 4.6 意图过滤与降级机制

#### 设计目标

**两层防护，职责分离**:

1. **Guard (前置过滤)**: 在意图分类层判断是否为网络运维相关请求
2. **Quick Query (默认降级)**: Skill 路由失败时的兜底执行策略

```
┌─────────────────────────────────────────────────────────────────────┐
│                    意图过滤 + Skill 路由 + 降级流程                   │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  用户输入                                                           │
│      │                                                              │
│      ▼                                                              │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 1: Guard 意图过滤               │  ← 前置过滤器          │
│  │     "这是网络运维相关问题吗？"            │                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│          ┌─────────┴─────────┐                                      │
│          │                   │                                      │
│          ▼                   ▼                                      │
│       是 (网络相关)        否 (非网络)                               │
│          │                   │                                      │
│          │                   ▼                                      │
│          │           ┌──────────────────┐                           │
│          │           │  礼貌拒绝         │                           │
│          │           │  "我是网络运维助手，│                          │
│          │           │   这个问题我帮不了" │                          │
│          │           └──────────────────┘                           │
│          │                                                          │
│          ▼                                                          │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 2: Skill 路由                   │  ← LLM 语义匹配        │
│  │     从 frontmatter 中选择最佳 Skill       │                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│          ┌─────────┴─────────┐                                      │
│          │                   │                                      │
│          ▼                   ▼                                      │
│      匹配成功              无匹配 (confidence < 阈值)                │
│          │                   │                                      │
│          ▼                   ▼                                      │
│  ┌───────────────┐   ┌──────────────────────────┐                   │
│  │ 加载执行 Skill │   │  Step 3: Fallback         │  ← 默认降级目标 │
│  │ (deep-analysis,│   │  执行 Quick Query Skill   │                 │
│  │  inspection...)│   │  (简单状态查询策略)        │                 │
│  └───────────────┘   └──────────────────────────┘                   │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

#### Guard 过滤器实现

```python
# 内置于 Agent 初始化，不作为独立 Skill

GUARD_PROMPT = """你是网络运维 AI 助手的意图过滤器。

## 任务
判断用户输入是否为网络运维相关请求。

## 网络运维相关 (返回 true)
- 查询设备状态 (接口、路由、BGP、OSPF)
- 故障排查 (ping 不通、丢包、延迟高)
- 配置检查 (VLAN、ACL、路由策略)
- 设备巡检 (健康检查、告警查询)
- 网络拓扑相关问题
- IP/MAC/ARP 查询
- 任何涉及网络设备的问题

## 非网络运维 (返回 false)
- 闲聊 ("你好"、"今天天气怎么样")
- 编程问题 ("写个 Python 脚本")
- 通用知识 ("什么是人工智能")
- 其他领域运维 ("数据库怎么优化")

## 用户输入
{user_query}

返回 JSON: {{"is_network_ops": true/false, "reason": "判断理由"}}"""

def guard_filter(user_query: str, llm: BaseChatModel) -> dict:
    """Guard 意图过滤 - 在进入 Skill 路由之前"""
    response = llm.invoke(GUARD_PROMPT.format(user_query=user_query))
    return json.loads(response.content)
```

#### Quick Query 作为默认降级 Skill

```markdown
# .olav/skills/quick-query.md
---
id: quick-query
intent: query
complexity: simple
is_fallback: true                    # ← 标记为降级目标
description: 简单状态查询，1-2条命令即可完成。无法匹配其他 Skill 时自动使用此策略。
examples:
  - 查看接口状态
  - R1 的 BGP 邻居
  - 看看路由表
---

# Quick Query (快速查询)

## 适用场景
...
```

#### 路由逻辑增强

```python
def route_to_skill(user_query: str, skill_index: dict, llm: BaseChatModel) -> dict:
    """Skill 路由 + 降级处理"""
    
    # Step 1: Guard 过滤
    guard_result = guard_filter(user_query, llm)
    if not guard_result["is_network_ops"]:
        return {
            "skill_id": None,
            "action": "reject",
            "message": f"抱歉，我是网络运维助手，这个问题不在我的能力范围内。{guard_result['reason']}"
        }
    
    # Step 2: Skill 路由
    routing_result = select_skill_with_confidence(user_query, skill_index, llm)
    
    # Step 3: 降级检查
    CONFIDENCE_THRESHOLD = 0.6
    if routing_result["confidence"] < CONFIDENCE_THRESHOLD:
        # 找到标记为 is_fallback: true 的 Skill
        fallback_skill = find_fallback_skill(skill_index)
        return {
            "skill_id": fallback_skill["id"],
            "action": "fallback",
            "original_confidence": routing_result["confidence"],
            "reason": f"无法确定最佳 Skill (confidence={routing_result['confidence']:.2f})，使用 {fallback_skill['id']} 策略"
        }
    
    return {
        "skill_id": routing_result["skill_id"],
        "action": "matched",
        "confidence": routing_result["confidence"]
    }


def select_skill_with_confidence(user_query: str, skill_index: dict, llm: BaseChatModel) -> dict:
    """带置信度的 Skill 选择"""
    prompt = f"""根据用户问题选择最合适的 Skill，并给出匹配置信度。

## 可用 Skills
{format_skill_descriptions(skill_index)}

## 用户问题
{user_query}

## 评分标准
- 1.0: 完美匹配 (问题与示例高度相似)
- 0.8: 高度匹配 (问题类型明确属于该 Skill)
- 0.6: 中等匹配 (可能适用但不确定)
- 0.4: 低匹配 (勉强相关)
- 0.2: 几乎不匹配

返回 JSON: 
{{"skill_id": "选中的skill", "confidence": 0.0-1.0, "reason": "选择理由"}}"""
    
    response = llm.invoke(prompt)
    return json.loads(response.content)


def find_fallback_skill(skill_index: dict) -> dict:
    """查找标记为 fallback 的 Skill"""
    for skill_id, meta in skill_index["skills"].items():
        if meta.get("is_fallback", False):
            return {"id": skill_id, **meta}
    
    # 如果没有显式标记，返回 quick-query
    return skill_index["skills"].get("quick-query", {"id": "quick-query"})
```

#### 完整流程示例

```
用户: "核心交换机的 BGP 邻居状态"
      │
      ├── Guard 过滤 → is_network_ops=true ✓
      │
      ├── Skill 路由 → skill_id=quick-query, confidence=0.92 ✓
      │
      └── 执行 quick-query 策略


用户: "写个 Python 脚本解析日志"
      │
      ├── Guard 过滤 → is_network_ops=false ✗
      │
      └── 返回: "抱歉，我是网络运维助手，编程问题不在我的能力范围内"


用户: "帮我看看网络有没有问题" (模糊请求)
      │
      ├── Guard 过滤 → is_network_ops=true ✓
      │
      ├── Skill 路由 → 无明确匹配, confidence=0.45
      │
      └── Fallback → 执行 quick-query 策略
          (可能先询问: "请问您想检查哪个设备？")
```

---

## 5. Knowledge 知识层

### 5.1 什么是 Knowledge

**Knowledge = 事实性信息**，Agent 需要了解的环境上下文。

### 5.2 Knowledge 示例

#### .olav/knowledge/aliases.md

```markdown
# 设备别名

Agent 在执行命令前应查阅此文件，将用户使用的别名转换为实际值。

| 别名 | 实际值 | 类型 | 平台 | 备注 |
|------|--------|------|------|------|
| 核心交换机 | 10.1.1.1 | device | cisco_ios | 数据中心核心 |
| 出口路由器 | 10.1.1.254 | device | cisco_ios | 互联网出口 |
| 上海专线 | GigabitEthernet0/0/1 | interface | - | R1 上的专线接口 |
| 办公网 | VLAN 100 | vlan | - | 办公区域 |

## 使用说明
- 当用户提到这些别名时，自动替换为实际值
- 如果用户使用了新的别名，询问含义后更新此文件
```

#### .olav/knowledge/conventions.md

```markdown
# 网络命名约定

## 设备命名
- `CS-<城市>-<编号>`: 核心交换机 (Core Switch)
- `DS-<城市>-<编号>`: 汇聚交换机 (Distribution Switch)  
- `AS-<楼层>-<编号>`: 接入交换机 (Access Switch)
- `R-<城市>-<编号>`: 路由器

## VLAN 规划
- VLAN 1-99: 管理用
- VLAN 100-199: 办公区
- VLAN 200-299: 生产区
- VLAN 300-399: DMZ

## IP 规划
- 10.1.0.0/16: 总部
- 10.2.0.0/16: 上海分部
- 10.3.0.0/16: 北京分部
```

#### .olav/knowledge/solutions/crc-errors.md

```markdown
# 案例: CRC 错误导致网络抖动

## 问题描述
用户反映网络时断时续，ping 丢包严重。

## 排查过程
1. show interfaces → 发现 Gi0/1 有大量 CRC 错误
2. show interfaces transceiver → RX power -18 dBm (偏低)
3. 检查光纤连接 → 发现光模块老化

## 根因
光模块老化，接收功率下降导致 CRC 错误。

## 解决方案
更换光模块。

## 关键命令
- show interfaces | include CRC|errors
- show interfaces transceiver detail

## 标签
#接口 #CRC #光模块 #物理层
```

---

## 6. Tools 能力层 (文件即真理)

### 6.1 设计理念

**核心思想**: 文件是唯一真理源，DuckDB 只是运行时缓存。

```
imports/                          ← 唯一真理源 (Git 版本控制)
├── commands/                     
│   ├── cisco_ios.txt            # ✅ 启用
│   ├── _huawei_vrp.txt          # ❌ 禁用 (_ 前缀)
│   └── juniper_junos.txt        
└── apis/                        
    ├── netbox.yaml              # OpenAPI 原生格式
    └── _zabbix.yaml             # ❌ 禁用
            ↓
      olav reload                # 手动刷新
            ↓
        DuckDB                   # 运行时缓存，可删除重建
```

**与 Skills/Knowledge 一致的模式**:

| 层 | 真理源 | 格式 | 禁用方式 |
|----|--------|------|----------|
| Skills | `.olav/skills/*.md` | Markdown | `_` 前缀 |
| Knowledge | `.olav/knowledge/*.md` | Markdown | `_` 前缀 |
| **Capabilities** | `.olav/imports/**` | txt/yaml | `_` 前缀 |

**用户操作**:

| 操作 | 方式 |
|------|------|
| 添加平台 | 创建 `.olav/imports/commands/arista.txt` |
| 禁用平台 | 重命名为 `_arista.txt` |
| 删除平台 | 删除文件 |
| 生效 | `olav reload` |

### 6.2 Capabilities 表设计 (统一 ETL)

**设计目标**: 统一管理 CLI 命令、API、NETCONF、gRPC 等多种能力类型。

```sql
-- DuckDB: .olav/capabilities.db (运行时缓存，可删除重建)

CREATE TABLE capabilities (
    id INTEGER PRIMARY KEY,
    
    -- 类型 (可扩展)
    type TEXT NOT NULL,              -- 'cli' | 'api' | 'netconf' | 'grpc' | ...
    
    -- 来源追溯
    source_file TEXT NOT NULL,       -- .olav/imports/commands/cisco_ios.txt
    source_type TEXT NOT NULL,       -- 'txt' | 'yaml' | 'openapi' | 'yang'
    
    -- 通用字段
    platform TEXT NOT NULL,          -- cisco_ios | netbox | openconfig
    name TEXT NOT NULL,              -- show interface* | /dcim/devices/
    description TEXT,
    
    -- 类型特定字段 (JSON 灵活存储)
    metadata JSON,                   -- {method: 'GET', params: {...}, xpath: '...'}
    
    -- 安全控制
    is_write BOOLEAN DEFAULT FALSE,
    requires_hitl BOOLEAN DEFAULT FALSE,
    
    -- 时间戳
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 索引优化
CREATE INDEX idx_cap_type ON capabilities(type);
CREATE INDEX idx_cap_platform ON capabilities(platform);
CREATE INDEX idx_cap_name ON capabilities(name);
```

### 6.3 统一 ETL 框架 (可扩展)

```python
from abc import ABC, abstractmethod
from pathlib import Path
from typing import Protocol
from dataclasses import dataclass

@dataclass
class Capability:
    """统一能力模型"""
    type: str
    platform: str
    name: str
    source_file: str
    source_type: str
    description: str | None = None
    metadata: dict | None = None
    is_write: bool = False
    requires_hitl: bool = False


class CapabilityLoader(Protocol):
    """能力加载器协议 - 可扩展新格式"""
    
    def supported_extensions(self) -> list[str]:
        """返回支持的文件扩展名"""
        ...
    
    def load(self, file_path: Path) -> list[Capability]:
        """从文件加载能力列表"""
        ...


class CLICommandLoader:
    """CLI 命令加载器 (*.txt)"""
    
    def supported_extensions(self) -> list[str]:
        return [".txt"]
    
    def load(self, file_path: Path) -> list[Capability]:
        platform = file_path.stem  # cisco_ios.txt → cisco_ios
        capabilities = []
        
        for line in file_path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
                continue
            
            is_write = line.startswith("!")
            cmd = line.lstrip("!")
            
            capabilities.append(Capability(
                type="cli",
                platform=platform,
                name=cmd,
                source_file=str(file_path),
                source_type="txt",
                is_write=is_write,
                requires_hitl=is_write,
            ))
        
        return capabilities


class OpenAPILoader:
    """OpenAPI 加载器 (*.yaml)"""
    
    def supported_extensions(self) -> list[str]:
        return [".yaml", ".yml"]
    
    def load(self, file_path: Path) -> list[Capability]:
        import yaml
        spec = yaml.safe_load(file_path.read_text())
        platform = file_path.stem
        capabilities = []
        
        for path, methods in spec.get("paths", {}).items():
            for method, details in methods.items():
                if method.startswith("x-"):
                    continue
                
                is_write = method.upper() in ("POST", "PUT", "PATCH", "DELETE")
                x_olav_write = details.get("x-olav-write", is_write)
                
                capabilities.append(Capability(
                    type="api",
                    platform=platform,
                    name=path,
                    source_file=str(file_path),
                    source_type="openapi",
                    description=details.get("summary"),
                    metadata={
                        "method": method.upper(),
                        "parameters": details.get("parameters", []),
                        "requestBody": details.get("requestBody"),
                    },
                    is_write=x_olav_write,
                    requires_hitl=x_olav_write,
                ))
        
        return capabilities


# 未来扩展示例
class YANGLoader:
    """YANG/NETCONF 加载器 (*.yang) - 未来实现"""
    
    def supported_extensions(self) -> list[str]:
        return [".yang"]
    
    def load(self, file_path: Path) -> list[Capability]:
        # 解析 YANG 模型，提取 XPath
        # 区分 config (read-write) 和 state (read-only)
        ...


class UnifiedETL:
    """统一 ETL 管理器"""
    
    def __init__(self):
        self.loaders: dict[str, CapabilityLoader] = {}
        
        # 注册默认加载器
        self.register(CLICommandLoader())
        self.register(OpenAPILoader())
    
    def register(self, loader: CapabilityLoader):
        """注册新的加载器"""
        for ext in loader.supported_extensions():
            self.loaders[ext] = loader
    
    def reload_all(self, imports_dir: Path = Path("imports")):
        """olav reload - 重建所有能力"""
        import duckdb
        
        conn = duckdb.connect("data/olav.db")
        conn.execute("DELETE FROM capabilities")
        
        total = 0
        for file_path in imports_dir.rglob("*"):
            # 跳过禁用文件 (_ 前缀)
            if file_path.name.startswith("_"):
                continue
            
            # 跳过目录和无扩展名文件
            if file_path.is_dir() or not file_path.suffix:
                continue
            
            ext = file_path.suffix.lower()
            if ext not in self.loaders:
                continue
            
            loader = self.loaders[ext]
            capabilities = loader.load(file_path)
            
            for cap in capabilities:
                conn.execute("""
                    INSERT INTO capabilities 
                    (type, platform, name, source_file, source_type, 
                     description, metadata, is_write, requires_hitl)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, [
                    cap.type, cap.platform, cap.name, cap.source_file,
                    cap.source_type, cap.description,
                    json.dumps(cap.metadata) if cap.metadata else None,
                    cap.is_write, cap.requires_hitl
                ])
                total += 1
        
        conn.close()
        print(f"✅ Loaded {total} capabilities from {imports_dir}")
        return total
```

### 6.4 扩展性设计

| 能力类型 | 加载器 | 源文件 | 状态 |
|---------|--------|--------|------|
| CLI 命令 | `CLICommandLoader` | `*.txt` | ✅ 已实现 |
| REST API | `OpenAPILoader` | `*.yaml` | ✅ 已实现 |
| NETCONF | `YANGLoader` | `*.yang` | 🔜 计划中 |
| gRPC | `gRPCLoader` | `*.proto` | 🔜 计划中 |
| GraphQL | `GraphQLLoader` | `*.graphql` | 🔜 计划中 |

**添加新能力类型**:

```python
# 1. 实现加载器
class MyLoader:
    def supported_extensions(self) -> list[str]:
        return [".myext"]
    
    def load(self, file_path: Path) -> list[Capability]:
        ...

# 2. 注册
etl = UnifiedETL()
etl.register(MyLoader())

# 3. 添加文件
# imports/my-capabilities/example.myext

# 4. 重新加载
# olav reload
```

### 6.5 search_capabilities 工具

```python
@tool
def search_capabilities(
    query: str,
    type: Literal["cli", "api", "netconf", "all"] = "all",
    platform: str | None = None,
    limit: int = 20
) -> list[dict]:
    """搜索可用的能力 (CLI 命令、API、NETCONF 等)
    
    在能力库中搜索匹配的能力。支持自然语言查询，
    LLM 可以用语义描述需求，工具返回匹配的能力。
    
    Args:
        query: 搜索关键词 (如 "接口状态", "bgp", "设备列表")
        type: 能力类型
            - "cli": CLI 命令
            - "api": REST API
            - "netconf": NETCONF XPath
            - "all": 搜索全部
        platform: 限定平台 (cisco_ios, huawei_vrp, netbox, openconfig)
        limit: 返回结果数量限制
    
    Returns:
        匹配的能力列表，每项包含:
        - type: cli | api | netconf
        - platform: 平台名称
        - name: 命令或端点
        - metadata: 类型特定信息 (method, xpath 等)
        - is_write: 是否为写操作
        - requires_hitl: 是否需要 HITL 审批
    """
    import duckdb
    import json
    
    conn = duckdb.connect("data/olav.db", read_only=True)
    
    sql = """
        SELECT type, platform, name, description, metadata, 
               is_write, requires_hitl, source_file
        FROM capabilities
        WHERE name ILIKE ? OR description ILIKE ? OR platform ILIKE ?
    """
    pattern = f"%{query}%"
    params = [pattern, pattern, pattern]
    
    if type != "all":
        sql += " AND type = ?"
        params.append(type)
    
    if platform:
        sql += " AND platform = ?"
        params.append(platform)
    
    sql += f" LIMIT {limit}"
    
    results = conn.execute(sql, params).fetchall()
    columns = ["type", "platform", "name", "description", "metadata", 
               "is_write", "requires_hitl", "source_file"]
    
    capabilities = []
    for row in results:
        cap = dict(zip(columns, row))
        # 解析 JSON metadata
        if cap["metadata"]:
            cap["metadata"] = json.loads(cap["metadata"])
        capabilities.append(cap)
    
    return capabilities
```

### 6.6 命令匹配逻辑

```python
def is_command_allowed(command: str, platform: str) -> bool:
    """检查命令是否在白名单中
    
    匹配规则:
    1. 精确匹配: "show version" 匹配 "show version"
    2. 通配符匹配: "show interface*" 匹配 "show interface GigabitEthernet0/1"
    """
    import duckdb
    
    conn = duckdb.connect("data/olav.db", read_only=True)
    cmd_lower = command.lower().strip()
    
    # 获取该平台所有命令模式
    patterns = conn.execute("""
        SELECT name FROM capabilities 
        WHERE type = 'command' AND platform = ?
    """, [platform]).fetchall()
    
    for (pattern,) in patterns:
        pattern = pattern.lower().strip()
        if pattern.endswith("*"):
            # 通配符匹配
            prefix = pattern[:-1]
            if cmd_lower.startswith(prefix):
                return True
        else:
            # 精确匹配
            if cmd_lower == pattern:
                return True
    
    return False
```

### 6.6 api_call 元工具

```python
@tool
def api_call(
    system: str,
    method: str,
    endpoint: str,
    params: dict | None = None,
    body: dict | None = None
) -> dict:
    """调用外部 API 系统
    
    使用前请先调用 search_capabilities 了解可用端点。
    
    Args:
        system: API 系统名 (netbox, zabbix)
        method: HTTP 方法 (GET, POST, PUT, PATCH, DELETE)
        endpoint: API 端点路径 (如 /dcim/devices/)
        params: URL 查询参数
        body: 请求体 (POST/PUT/PATCH)
    
    Returns:
        API 响应数据
    
    Examples:
        # 查询设备列表
        api_call("netbox", "GET", "/dcim/devices/", params={"name": "R1"})
        
        # 更新设备状态 (需要 HITL 审批)
        api_call("netbox", "PATCH", "/dcim/devices/1/", body={"status": "active"})
    """
```

### 6.7 数据管理

**唯一命令**: `olav reload`

```bash
# 从 imports/ 目录重建 DuckDB 缓存
olav reload

# 预览变更 (不实际执行)
olav reload --dry-run

# 验证文件格式
olav validate
```

#### reload 逻辑

```python
def reload_capabilities(imports_dir: Path = Path(".olav/imports")):
    """从文件重建能力数据库
    
    1. 清空 capabilities 表
    2. 扫描 .olav/imports/commands/*.txt (跳过 _ 前缀)
    3. 扫描 .olav/imports/apis/*.yaml (跳过 _ 前缀)
    4. 解析并插入 DuckDB
    """
    conn = duckdb.connect(".olav/capabilities.db")
    conn.execute("DELETE FROM capabilities")
    
    # 加载命令文件
    for txt_file in (imports_dir / "commands").glob("*.txt"):
        if txt_file.name.startswith("_"):
            continue  # 跳过禁用文件
        platform = txt_file.stem  # cisco_ios.txt → cisco_ios
        load_commands(conn, txt_file, platform)
    
    # 加载 API 文件
    for yaml_file in (imports_dir / "apis").glob("*.yaml"):
        if yaml_file.name.startswith("_"):
            continue
        platform = yaml_file.stem  # netbox.yaml → netbox
        load_openapi(conn, yaml_file, platform)
    
    conn.close()
    print(f"Reloaded {count} capabilities")
```

#### 命令文件格式 (txt)

```txt
# .olav/imports/commands/cisco_ios.txt
# 每行一条命令，支持通配符和注释

# === 基础信息 ===
show version
show inventory

# === 接口相关 ===
show interface*
show ip interface brief

# === 路由相关 ===
show ip route*
show ip bgp*
show ip ospf*

# === 配置命令 (需要 HITL，用 ! 前缀标记) ===
!configure terminal
!write memory
```

#### API 文件格式 (OpenAPI YAML)

```yaml
# .olav/imports/apis/netbox.yaml
# 标准 OpenAPI 3.0 格式，额外支持 x-olav 扩展

openapi: "3.0.0"
info:
  title: NetBox API
  version: "3.0"
paths:
  /dcim/devices/:
    get:
      summary: 查询设备列表
      x-olav-write: false  # 可选: 标记是否需要 HITL
  /dcim/devices/{id}/:
    patch:
      summary: 更新设备信息
      x-olav-write: true   # 需要 HITL 审批
```

### 6.8 审计日志表

```sql
CREATE TABLE audit_logs (
    id INTEGER PRIMARY KEY,
    timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    thread_id TEXT NOT NULL,
    device TEXT NOT NULL,
    command TEXT NOT NULL,
    output TEXT,
    success BOOLEAN NOT NULL,
    duration_ms INTEGER,
    user TEXT
);

CREATE INDEX idx_audit_thread ON audit_logs(thread_id);
CREATE INDEX idx_audit_device ON audit_logs(device);
```

**注意**: 不使用命令缓存，始终获取最新数据。网络状态变化快，缓存可能导致误判。

### 6.9 Nornir 执行策略 (不重复造轮子)

**原则**: 错误处理、超时控制、并发策略全部使用 Nornir 原生机制。

#### Nornir 配置

```yaml
# .olav/config/nornir/config.yaml
runner:
  plugin: threaded
  options:
    num_workers: 20        # 并发执行数

inventory:
  plugin: SimpleInventory
  options:
    host_file: ".olav/config/nornir/hosts.yaml"

# 连接参数 (全局默认)
connection_options:
  netmiko:
    extras:
      timeout: 60          # 连接超时
      auth_timeout: 30     # 认证超时
      banner_timeout: 30   # Banner 超时
      conn_timeout: 30     # TCP 连接超时
```

#### 错误处理

```python
from nornir_utils.plugins.functions import print_failed_hosts

def execute_with_error_handling(nr: Nornir, task, *args, **kwargs):
    """使用 Nornir 原生错误处理"""
    result = nr.run(task=task, *args, **kwargs)
    
    # Nornir 自动收集失败主机
    if result.failed:
        failed_hosts = list(result.failed_hosts.keys())
        for host in failed_hosts:
            error = result[host].exception
            log.error(f"{host}: {error}")
    
    return result
```

#### 超时控制

```python
from nornir_netmiko.tasks import netmiko_send_command

def send_command_with_timeout(task, command: str, timeout: int = 30):
    """使用 Netmiko 原生超时"""
    return task.run(
        task=netmiko_send_command,
        command_string=command,
        read_timeout=timeout,      # 读取超时
        cmd_verify=True,           # 命令确认
    )
```

#### 并发控制

```python
from nornir import InitNornir
from nornir.core.filter import F

# 按需调整并发数
nr = InitNornir(
    runner={"plugin": "threaded", "options": {"num_workers": 10}}
)

# 或使用过滤器限制范围
core_devices = nr.filter(F(role="core"))
result = core_devices.run(task=some_task)
```

#### 重试机制

```python
from nornir_utils.plugins.tasks import retry

@retry(num_retries=3, retry_delay=5)
def robust_command(task, command: str):
    """Nornir 原生重试装饰器"""
    return task.run(
        task=netmiko_send_command,
        command_string=command,
    )
```

---

## 7. DeepAgents 框架集成

> **核心定位**: OLAV 完全构建于 DeepAgents 框架之上，充分利用其原生能力，避免重复造轮子。

### 7.1 Agent Harness 内置能力

DeepAgents 提供了完整的 Agent 运行时环境，OLAV 直接利用以下内置能力：

| 能力 | DeepAgents 实现 | OLAV 应用场景 |
|------|----------------|--------------|
| **write_todos** | 任务分解与追踪 | 深度分析时自动规划诊断步骤 |
| **Filesystem** | ls/read/write/edit/glob/grep | 读写 Skills/Knowledge/Tools 文件 |
| **Subagent (task)** | 上下文隔离的子任务委派 | config-analyzer, topology-explorer |
| **Large Result Eviction** | >20k tokens 自动写入文件 | 处理 `show running-config` 等大输出 |
| **History Summarization** | 自动压缩长对话 | 长时间排查会话保持上下文 |

### 7.2 HITL (Human-in-the-Loop) 机制

DeepAgents 通过 `interrupt_on` 参数原生支持 HITL：

```python
from deepagents import create_deep_agent

# HITL 配置 - 写操作需要人工审批
HITL_TOOLS = [
    "nornir_execute",      # 任何设备写操作
    "write_file",          # 文件写入
    "edit_file",           # 文件编辑
]

agent = create_deep_agent(
    model=llm,
    tools=all_tools,
    system_prompt=OLAV_SYSTEM_PROMPT,
    interrupt_on=HITL_TOOLS,  # 遇到这些工具时暂停等待审批
    checkpointer=checkpointer,  # 状态持久化，支持中断恢复
)
```

**工作流程**:
1. Agent 调用 `nornir_execute` 准备执行写命令
2. DeepAgents 自动暂停，返回 interrupt 状态
3. 前端展示待执行命令，等待用户确认
4. 用户批准后，使用 `Command(resume=...)` 继续执行

### 7.3 Checkpointer 状态持久化 ✅ COMPLETED

**状态**: ✅ 已实现 JSON 持久化 (v0.8-deepagents)

```python
# 现在使用基于文件系统的 JSON 持久化，支持跨会话恢复
# 存储路径: .olav/.agent_memory.json
```

**当前支持**:
- ✅ HITL 中断期间状态保持
- ✅ 跨会话恢复 (Agent Memory)
- ✅ 历史统计与摘要

### 7.4 Storage Backend 策略 (CompositeBackend)

DeepAgents 支持 CompositeBackend 实现路径级别的存储策略：

```
┌─────────────────────────────────────────────────────────────┐
│                    CompositeBackend                          │
│                                                             │
│  路径                         → 后端                → 特性   │
│  ───────────────────────────────────────────────────────────│
│  .olav/skills/*              → StoreBackend (持久化)→ 需审批 │
│  .olav/knowledge/*           → StoreBackend (持久化)→ 需审批 │
│  .olav/imports/commands/     → StoreBackend (持久化)→ 需审批 │
│  .olav/imports/apis/         → StoreBackend (持久化)→ 只读   │
│  /scratch/*                  → StateBackend (临时)  → 会话内 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**关键设计**: Agent 对 `.olav/skills/`, `.olav/knowledge/`, `.olav/imports/commands/` 可请求写入，但所有修改需要 HITL 审批。

### 7.5 Subagent 简化设计 (宏观/微观置信度模型)

**设计原则**: 基于 Nornir 统一执行层的两阶段置信度诊断模型。

> **架构变更说明**: SuzieQ 已从架构中移除，宏观分析改为使用 Nornir 执行**轻量级诊断命令**。

#### 置信度模型

```python
# 置信度阈值常量
MIN_ACCEPTABLE_CONFIDENCE = 0.50   # 50% - 单层最低置信度
MACRO_MAX_CONFIDENCE = 0.70        # 70% - 宏观分析 (快速命令) 置信度上限
MICRO_MAX_CONFIDENCE = 0.95        # 95% - 微观分析 (深度诊断) 置信度上限
ROOT_CAUSE_THRESHOLD = 0.90        # 90% - 确定根因的最低置信度
```

#### 宏观分析命令集 (分层)

宏观分析使用**只读、快速、低风险**的诊断命令，按网络层分类：

```yaml
# .olav/config/diagnosis/macro_commands.yaml
L1_Physical:
  description: "物理层快速诊断"
  commands:
    - cmd: "show interfaces status"
      purpose: "接口状态总览 (up/down/err-disabled)"
      timeout: 10
    - cmd: "show interfaces counters errors"
      purpose: "错误计数器 (CRC/collisions/drops)"
      timeout: 10
    - cmd: "show environment all"
      purpose: "硬件健康 (温度/电源/风扇)"
      timeout: 15
  confidence_factors:
    - "error_counters > 0 → +20%"
    - "interface_down → +25%"
    - "environment_warning → +15%"

L2_DataLink:
  description: "链路层快速诊断"
  commands:
    - cmd: "show mac address-table count"
      purpose: "MAC 表规模 (溢出检测)"
      timeout: 10
    - cmd: "show spanning-tree summary"
      purpose: "STP 状态 (root/blocking)"
      timeout: 10
    - cmd: "show lldp neighbors"
      purpose: "邻居发现 (拓扑验证)"
      timeout: 15
  confidence_factors:
    - "mac_table_full → +30%"
    - "stp_blocking → +20%"
    - "lldp_mismatch → +15%"

L3_Network:
  description: "网络层快速诊断"
  commands:
    - cmd: "show ip route summary"
      purpose: "路由表规模 (收敛验证)"
      timeout: 10
    - cmd: "show ip bgp summary"
      purpose: "BGP 邻居状态"
      timeout: 15
    - cmd: "show ip ospf neighbor"
      purpose: "OSPF 邻居状态"
      timeout: 15
    - cmd: "show arp summary"
      purpose: "ARP 表规模"
      timeout: 10
  confidence_factors:
    - "bgp_neighbor_down → +30%"
    - "ospf_neighbor_missing → +25%"
    - "route_count_changed → +20%"

L4_Transport:
  description: "传输层快速诊断"
  commands:
    - cmd: "show ip nat translations total"
      purpose: "NAT 会话数"
      timeout: 10
    - cmd: "show access-lists | include matches"
      purpose: "ACL 命中统计"
      timeout: 15
  confidence_factors:
    - "nat_exhaustion → +25%"
    - "acl_deny_spike → +20%"
```

#### 两阶段诊断架构 (Nornir 统一执行)

```
┌─────────────────────────────────────────────────────────────┐
│              两阶段置信度诊断流程 (Nornir 执行)              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Phase 1: 宏观分析 (MacroAnalyzer)                          │
│  ───────────────────────────────────────────────────────── │
│  执行方式: Nornir 批量执行轻量级命令                         │
│  命令来源: .olav/config/diagnosis/macro_commands.yaml       │
│  特点: 只读、快速 (<30s)、低风险、可并行                     │
│  置信度上限: 70% (MACRO_MAX_CONFIDENCE)                     │
│                                                             │
│  执行流程:                                                  │
│  1. 根据用户问题确定目标设备 (从 NetBox 查询)                │
│  2. 并行执行所有层的快速诊断命令                            │
│  3. LLM 分析命令输出，评估各层置信度                        │
│                                                             │
│  输出: 各层置信度 + 理由                                    │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ L1 (物理层):  55% - Gi0/1 CRC errors: 1523           │  │
│  │ L2 (链路层):  40% - MAC table: 2341/32768, STP OK    │  │
│  │ L3 (网络层):  70% - BGP neighbor 10.1.1.2 is Down    │  │
│  │ L4 (传输层):  30% - NAT translations: 1.2k, normal   │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
│  判断: max(55%, 40%, 70%, 30%) = 70% < 90%                  │
│  → 触发 Phase 2 (优先分析 L3)                               │
│                                                             │
│  ═══════════════════════════════════════════════════════   │
│                                                             │
│  Phase 2: 微观分析 (MicroAnalyzer)                          │
│  ───────────────────────────────────────────────────────── │
│  触发条件: Phase 1 max(置信度) < 90%                         │
│  执行方式: Nornir 执行深度诊断命令                           │
│  工具: nornir_cli (show commands) + nornir_netconf (get)    │
│  置信度上限: 95% (MICRO_MAX_CONFIDENCE)                     │
│                                                             │
│  按 Phase 1 置信度排序，优先分析最可疑层:                    │
│  1. L3 (70%) → show ip bgp neighbor 10.1.1.2               │
│                show ip bgp 10.0.0.0/8                       │
│                show run | section router bgp                │
│  2. L1 (55%) → show interfaces Gi0/1                       │
│                show controllers Gi0/1                       │
│                                                             │
│  输出: 根因确认 (≥90% 置信度)                               │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ 根因: BGP neighbor 10.1.1.2 配置了错误的 remote-as    │  │
│  │ 置信度: 92%                                           │  │
│  │ 证据: show ip bgp neighbor 显示 AS mismatch           │  │
│  │       show run 确认 neighbor 10.1.1.2 remote-as 65001 │  │
│  │       (正确值应为 65002)                               │  │
│  └───────────────────────────────────────────────────────┘  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 宏观分析命令执行 (Nornir 并行)

```python
from nornir import InitNornir
from nornir_netmiko.tasks import netmiko_send_command
from nornir.core.filter import F
import yaml

class MacroAnalyzer:
    """Phase 1 宏观分析器 - 使用 Nornir 执行轻量级诊断命令"""
    
    MACRO_MAX_CONFIDENCE = 0.70  # 宏观分析置信度上限
    
    def __init__(self, config_path: str = ".olav/config/diagnosis/macro_commands.yaml"):
        self.nr = InitNornir(config_file=".olav/config/nornir/config.yaml")
        with open(config_path) as f:
            self.commands_config = yaml.safe_load(f)
    
    async def execute(self, devices: list[str], problem_description: str) -> dict:
        """执行宏观分析
        
        Args:
            devices: 目标设备列表 (hostname)
            problem_description: 用户问题描述
        
        Returns:
            各层置信度分析结果
        """
        # 1. 过滤目标设备
        target_nr = self.nr.filter(F(name__any=devices))
        
        # 2. 收集所有层的命令
        all_results = {}
        for layer, config in self.commands_config.items():
            layer_results = []
            for cmd_info in config["commands"]:
                result = target_nr.run(
                    task=netmiko_send_command,
                    command_string=cmd_info["cmd"],
                    read_timeout=cmd_info.get("timeout", 30),
                )
                layer_results.append({
                    "command": cmd_info["cmd"],
                    "purpose": cmd_info["purpose"],
                    "output": self._extract_outputs(result),
                })
            all_results[layer] = layer_results
        
        # 3. LLM 分析输出，评估置信度
        analysis = await self._analyze_with_llm(
            all_results, 
            problem_description,
            self.commands_config
        )
        
        # 4. 限制置信度上限
        for layer in analysis["layer_analysis"]:
            if analysis["layer_analysis"][layer]["confidence"] > self.MACRO_MAX_CONFIDENCE:
                analysis["layer_analysis"][layer]["confidence"] = self.MACRO_MAX_CONFIDENCE
                analysis["layer_analysis"][layer]["capped"] = True
        
        return analysis
    
    def _extract_outputs(self, nornir_result) -> dict[str, str]:
        """提取 Nornir 结果"""
        return {
            host: result.result if not result.failed else f"ERROR: {result.exception}"
            for host, result in nornir_result.items()
        }
    
    async def _analyze_with_llm(self, results: dict, problem: str, config: dict) -> dict:
        """使用 LLM 分析命令输出"""
        prompt = f"""分析以下网络诊断命令输出，评估各层与问题的相关性。

## 用户问题
{problem}

## 命令输出
{json.dumps(results, indent=2, ensure_ascii=False)}

## 置信度评估指南
{yaml.dump(config, allow_unicode=True)}

## 输出要求 (JSON)
{{
  "layer_analysis": {{
    "L1_Physical": {{"confidence": 0.0-0.70, "reason": "..."}},
    "L2_DataLink": {{"confidence": 0.0-0.70, "reason": "..."}},
    "L3_Network": {{"confidence": 0.0-0.70, "reason": "..."}},
    "L4_Transport": {{"confidence": 0.0-0.70, "reason": "..."}}
  }},
  "priority_layer": "最可疑的层",
  "suspected_devices": ["可疑设备列表"],
  "hypothesis": "初步假设"
}}

注意: 置信度上限为 70%，因为这是快速诊断，需要 Phase 2 深度验证。"""
        
        return await self.llm.ainvoke(prompt)
```

#### Subagent 定义 (Nornir 工具)

```python
from deepagents.middleware.subagents import SubAgentMiddleware

subagent_middleware = SubAgentMiddleware(
    subagents=[
        {
            "name": "macro-analyzer",
            "description": "Phase 1 宏观分析: 使用 Nornir 执行快速诊断命令，评估 L1-L4 各层置信度",
            "system_prompt": """你是网络宏观分析专家，执行 Phase 1 快速扫描。

## 核心约束
- 使用 Nornir 执行轻量级诊断命令 (只读、快速)
- 置信度上限: 70% (不能超过此值)
- 必须给出每层的置信度和理由

## 诊断命令分类
- L1 物理层: show interfaces status/counters/environment
- L2 链路层: show mac address-table/spanning-tree/lldp
- L3 网络层: show ip route/bgp/ospf/arp summary
- L4 传输层: show ip nat/access-lists

## 输出格式 (JSON)
{
  "layer_analysis": {
    "L1_Physical": {"confidence": 0.55, "reason": "CRC errors on Gi0/1"},
    "L2_DataLink": {"confidence": 0.40, "reason": "MAC/STP normal"},
    "L3_Network": {"confidence": 0.70, "reason": "BGP neighbor down"},
    "L4_Transport": {"confidence": 0.30, "reason": "NAT/ACL normal"}
  },
  "priority_layer": "L3_Network",
  "suspected_devices": ["R1", "R2"],
  "hypothesis": "BGP 邻居状态异常导致路由缺失"
}

## 工作流程
1. 从 NetBox 查询目标设备
2. 使用 nornir_execute 批量执行诊断命令
3. 分析输出，评估各层置信度 (max 70%)
4. 输出 priority_layer 供 Phase 2 参考""",
            "tools": ["netbox_query", "nornir_execute"],
        },
        {
            "name": "micro-analyzer", 
            "description": "Phase 2 微观分析: 深度诊断，确认根因 (≥90%)",
            "system_prompt": """你是网络微观分析专家，执行 Phase 2 深度诊断。

## 核心约束
- 使用 Nornir 执行针对性深度诊断命令
- 目标: 置信度达到 90% 确认根因
- 按 Phase 1 的 priority_layer 顺序分析

## 深度诊断命令 (按层)
### L1 物理层深度诊断
- show interfaces <intf> (完整接口状态)
- show controllers <intf> (硬件级别信息)
- show cable-diagnostics tdr interface <intf> (线缆诊断)
- show transceiver details (光模块详情)

### L2 链路层深度诊断
- show mac address-table interface <intf>
- show spanning-tree interface <intf> detail
- show vlan id <vlan>
- show etherchannel summary

### L3 网络层深度诊断
- show ip bgp neighbor <ip> (完整 BGP 邻居状态)
- show ip bgp <prefix> (特定前缀详情)
- show ip ospf neighbor <router-id> detail
- show ip route <prefix> longer-prefixes
- show run | section router bgp

### L4 传输层深度诊断
- show ip nat translations verbose
- show access-lists <acl-name>
- show policy-map interface <intf>

## 输入
- Phase 1 分析结果 (从 /scratch/macro-result.json 读取)
- 包含各层置信度和 suspected_devices

## 输出格式 (JSON)
{
  "root_cause_found": true,
  "root_cause": "BGP neighbor 10.1.1.2 配置了错误的 remote-as 65001 (应为 65002)",
  "confidence": 0.92,
  "layer": "L3_Network",
  "evidence": [
    "show ip bgp neighbor 10.1.1.2: state=Idle, AS mismatch",
    "show run | section router bgp: neighbor 10.1.1.2 remote-as 65001"
  ],
  "remediation": "修改 BGP 配置: neighbor 10.1.1.2 remote-as 65002"
}

## 分析顺序
按 Phase 1 置信度从高到低排序，优先验证最可疑的层。
直到找到 ≥90% 置信度的根因或遍历所有层。""",
            "tools": ["nornir_execute", "nornir_netconf_get"],
        },
    ]
)
```

#### 深度诊断命令配置

```yaml
# .olav/config/diagnosis/micro_commands.yaml
L1_Physical:
  description: "物理层深度诊断"
  commands:
    - pattern: "show interfaces {interface}"
      purpose: "完整接口状态和计数器"
      requires: ["interface"]
    - pattern: "show controllers {interface}"
      purpose: "硬件控制器级别信息"
      requires: ["interface"]
    - pattern: "show transceiver interface {interface} detail"
      purpose: "光模块详细信息 (DDM)"
      requires: ["interface"]
      
L2_DataLink:
  description: "链路层深度诊断"
  commands:
    - pattern: "show mac address-table interface {interface}"
      purpose: "特定接口 MAC 表"
      requires: ["interface"]
    - pattern: "show spanning-tree interface {interface} detail"
      purpose: "STP 端口详细状态"
      requires: ["interface"]
    - pattern: "show vlan id {vlan_id}"
      purpose: "VLAN 详细信息"
      requires: ["vlan_id"]

L3_Network:
  description: "网络层深度诊断"
  commands:
    - pattern: "show ip bgp neighbor {neighbor_ip}"
      purpose: "BGP 邻居完整状态"
      requires: ["neighbor_ip"]
    - pattern: "show ip bgp {prefix}"
      purpose: "特定前缀 BGP 详情"
      requires: ["prefix"]
    - pattern: "show ip ospf neighbor {router_id} detail"
      purpose: "OSPF 邻居详细状态"
      requires: ["router_id"]
    - pattern: "show ip route {prefix} longer-prefixes"
      purpose: "路由详细信息"
      requires: ["prefix"]
    - pattern: "show run | section router bgp"
      purpose: "BGP 运行配置"
      requires: []
    - pattern: "show run | section router ospf"
      purpose: "OSPF 运行配置"
      requires: []

L4_Transport:
  description: "传输层深度诊断"
  commands:
    - pattern: "show ip nat translations verbose"
      purpose: "NAT 转换详情"
      requires: []
    - pattern: "show access-lists {acl_name}"
      purpose: "ACL 详细信息"
      requires: ["acl_name"]
```

#### 状态共享机制 (文件系统)

```
/scratch/                      ← 会话内共享目录
├── macro-result.json          ← Phase 1 输出，Phase 2 读取
│   {
│     "layer_analysis": {...},
│     "priority_layer": "L3_Network",
│     "suspected_devices": ["R1", "R2"],
│     "hypothesis": "..."
│   }
└── micro-result.json          ← Phase 2 输出
    {
      "root_cause_found": true,
      "root_cause": "...",
      "confidence": 0.92,
      "evidence": [...]
    }
```

#### 使用场景

| 任务类型 | Subagent | 置信度目标 | 执行方式 |
|----------|----------|----------|----------|
| 快速查询 | ❌ 不需要 | - | 主 Agent 直接执行单条命令 |
| 设备巡检 | ❌ 不需要 | - | 主 Agent 按模板批量执行 |
| 故障分析 Phase 1 | macro-analyzer | ≤70% | Nornir 批量执行快速诊断命令 |
| 故障分析 Phase 2 | micro-analyzer | ≥90% | Nornir 执行针对性深度诊断 |

#### 为什么置信度上限是 70% (宏观) 和 95% (微观)?

| 阶段 | 置信度上限 | 原因 |
|------|----------|------|
| 宏观 (Phase 1) | 70% | 快速命令只能发现**症状**，无法确定**根因** |
| 微观 (Phase 2) | 95% | 深度诊断可查看配置和详细状态，但仍需人工确认 |
| 人工确认 | 100% | 只有人类可以做出最终决策和配置变更 |

### 7.6 Long-term Memory (长期记忆)

DeepAgents 支持多种记忆系统，OLAV 采用文件系统方式：

| 记忆类型 | 存储位置 | 内容 | 示例 |
|---------|---------|------|------|
| **Semantic Memory** | `.olav/knowledge/aliases.md` | 语义别名 | `core` = `R1, R2, R3, R4` |
| **Semantic Memory** | `.olav/knowledge/conventions.md` | 网络约定 | 管理网段 `10.255.0.0/24` |
| **Episodic Memory** | `.olav/knowledge/solutions/` | 成功案例 | BGP 邻居建立失败的排查过程 |
| **Procedural Memory** | `.olav/skills/*.md` | 执行策略 | 如何执行快速查询 |

**与传统 RAG 的区别**:
- **不依赖向量数据库** - 使用 Agent 原生的 `read_file`/`grep` 能力
- **Agent 可自学习** - 成功诊断后自动更新 `.olav/knowledge/solutions/`
- **人类可审计** - 所有记忆都是可读的 Markdown 文件

### 7.7 Dynamic Tool Loading (动态工具加载)

DeepAgents 支持运行时加载工具，OLAV 利用此能力实现：

```python
# 1. 从 .olav/imports/commands/*.txt 读取命令白名单
# 2. 从 .olav/imports/apis/*.yaml 读取 OpenAPI 定义
# 3. 动态生成 @tool 装饰的函数

def load_tools_from_directory(path: Path) -> list[Tool]:
    tools = []
    
    # 命令白名单 → CLI 工具
    for txt_file in path.glob("commands/*.txt"):
        commands = txt_file.read_text().strip().split("\n")
        tools.append(create_cli_tool(txt_file.stem, commands))
    
    # OpenAPI 定义 → API 工具
    for yaml_file in path.glob("apis/*.yaml"):
        spec = yaml.safe_load(yaml_file.read_text())
        tools.extend(create_api_tools(spec))
    
    return tools
```

**热加载能力**: 用户添加新的 `.txt` 或 `.yaml` 文件后，重启 Agent 即可使用新工具。

### 7.8 API 凭证管理 (DeepAgents 原生)

**原则**: 使用 DeepAgents 的原生凭证管理，不重复造轮子。

#### 凭证配置

```python
# .env 文件 (不提交 Git)
NETBOX_URL=https://netbox.example.com
NETBOX_TOKEN=your-api-token

ZABBIX_URL=https://zabbix.example.com
ZABBIX_USER=api-user
ZABBIX_PASSWORD=api-password

# Nornir 设备凭证
NORNIR_USERNAME=admin
NORNIR_PASSWORD=admin-password
```

#### DeepAgents 凭证注入

```python
from deepagents.tools import create_api_tool
from deepagents.auth import EnvCredentialProvider

# DeepAgents 原生凭证提供者
credential_provider = EnvCredentialProvider(
    mapping={
        "netbox": {
            "url": "NETBOX_URL",
            "token": "NETBOX_TOKEN",
        },
        "zabbix": {
            "url": "ZABBIX_URL",
            "username": "ZABBIX_USER",
            "password": "ZABBIX_PASSWORD",
        },
    }
)

# 工具自动获取凭证
api_call_tool = create_api_tool(
    name="api_call",
    credential_provider=credential_provider,
)
```

#### 安全边界

| 组件 | 访问凭证 | 说明 |
|------|---------|------|
| Agent | ❌ 不可见 | 不能读取 .env |
| Tools | ✅ 自动注入 | 通过 CredentialProvider |
| Logs | ❌ 脱敏 | 密码不记录到审计日志 |

---

## 8. Agentic 自学习机制

### 8.1 知识修改审批机制 (防止知识毒化)

**风险**: Agent 可能产生幻觉并写入错误知识（如 "遇到丢包就重启接口"），污染后续操作。

**解决方案**: 所有知识修改必须经过 HITL 审批。

```
┌─────────────────────────────────────────────────────────────┐
│              知识写入的 HITL 审批流程                        │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  Agent 发起写入请求                                         │
│       ↓                                                     │
│  interrupt_on 触发暂停                                      │
│       ↓                                                     │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  审批界面显示:                                       │    │
│  │                                                     │    │
│  │  📝 Agent 请求修改知识库                            │    │
│  │  ─────────────────────────────────────────────────  │    │
│  │  目标: .olav/knowledge/aliases.md                  │    │
│  │  操作: 添加别名 "核心交换机" = 10.1.1.1            │    │
│  │                                                     │    │
│  │  [✅ 批准]  [✏️ 编辑]  [❌ 拒绝]                    │    │
│  │                                                     │    │
│  └─────────────────────────────────────────────────────┘    │
│       ↓                                                     │
│  用户决策                                                   │
│  ├── 批准 → 执行写入                                        │
│  ├── 编辑 → 用户修改后写入                                  │
│  └── 拒绝 → 取消操作                                        │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 权限矩阵 (全部需要审批)

| 路径 | Agent 读 | Agent 写 | 审批要求 |
|------|---------|---------|----------|
| `.olav/skills/*.md` | ✅ | ⚠️ 需审批 | 🚫 需专家审批 |
| `.olav/knowledge/aliases.md` | ✅ | ⚠️ 需审批 | ⚠️ 需人工审批 |
| `.olav/knowledge/conventions.md` | ✅ | ⚠️ 需审批 | ⚠️ 需人工审批 |
| `.olav/knowledge/solutions/*.md` | ✅ | ⚠️ 需审批 | ⚠️ 需人工审批 |
| `.olav/imports/commands/*.txt` | ✅ | ⚠️ 需审批 | 🚫 需专家审批 |
| `.olav/imports/apis/*.yaml` | ✅ | ❌ 禁止 | - |
| `.olav/OLAV.md` | ✅ | ❌ 禁止 | - |
| `.env` | ❌ | ❌ 禁止 | - |

#### DeepAgents interrupt_on 配置

```python
from deepagents import create_deep_agent

# 定义需要审批的路径模式
KNOWLEDGE_PATHS = [".olav/skills/", ".olav/knowledge/", ".olav/imports/commands/"]

def requires_approval(tool_name: str, tool_args: dict) -> bool:
    """判断是否需要 HITL 审批"""
    if tool_name not in ["write_file", "edit_file"]:
        return False
    path = tool_args.get("path", "")
    return any(p in path for p in KNOWLEDGE_PATHS)

agent = create_deep_agent(
    model=llm,
    tools=all_tools,
    system_prompt=OLAV_SYSTEM_PROMPT,
    interrupt_on={
        "write_file": {
            "condition": lambda args: requires_approval("write_file", args),
            "allowed_decisions": ["approve", "edit", "reject"]
        },
        "edit_file": {
            "condition": lambda args: requires_approval("edit_file", args),
            "allowed_decisions": ["approve", "edit", "reject"]
        }
    },
    checkpointer=checkpointer,
)
```

### 8.2 Agent 可请求修改的内容

| 内容 | Agent 权限 | 审批级别 | 场景 |
|------|-----------|---------|------|
| `.olav/skills/*.md` | 读 + 请求写 | 🚫 专家审批 | 学习新的任务执行模式 |
| `.olav/knowledge/aliases.md` | 读 + 请求写 | ⚠️ 人工审批 | 记录设备别名 |
| `.olav/knowledge/solutions/*.md` | 读 + 请求写 | ⚠️ 人工审批 | 保存成功案例 |
| `.olav/imports/commands/*.txt` | 读 + 请求写 | 🚫 专家审批 | 发现新平台/新命令 |
| `.olav/imports/apis/*.yaml` | 只读 | - | API 定义由人类维护 |
| `.olav/OLAV.md` | 只读 | - | 核心规则由人类维护 |
| `.env` | 不可访问 | - | 敏感配置 |

### 8.3 学习场景示例 (带审批)

#### 场景 1: 学习设备别名 (需人工审批)

```
用户: 查一下核心交换机的状态
Agent: 请问"核心交换机"指的是哪台设备？
用户: 10.1.1.1

Agent 内心:
1. 我应该记住这个别名
2. edit_file .olav/knowledge/aliases.md
   添加: | 核心交换机 | 10.1.1.1 | device | cisco_ios |
   
   ⏸️ [HITL 暂停 - 等待审批]
   
   📝 Agent 请求修改: .olav/knowledge/aliases.md
   内容: 添加别名 "核心交换机" = 10.1.1.1
   
   用户: [✅ 批准]
   
   → 修改已应用

下次:
用户: 核心交换机的 CPU 使用率
Agent: (读取 aliases.md) 核心交换机 = 10.1.1.1
       执行 show processes cpu ...
```

#### 场景 2: 发现新平台命令 (需专家审批)

```
用户: 连到 10.2.2.2，这是华为设备，看接口状态
Agent:
1. ls .olav/imports/commands/ → 没有 huawei_vrp.txt
2. write_file .olav/imports/commands/huawei_vrp.txt

   # 华为 VRP 命令白名单
   display version
   display interface brief
   display ip interface brief
   display ip routing-table
   display current-configuration

   ⏸️ [HITL 暂停 - 需要专家审批]
   
   🚫 Agent 请求创建: .olav/imports/commands/huawei_vrp.txt
   风险级别: 高 (新增命令白名单)
   
   专家审核内容:
   - 确认命令是只读操作
   - 确认无危险命令 (reload, erase 等)
   
   专家: [✅ 批准] / [✏️ 编辑] / [❌ 拒绝]
   
3. 审批通过后:
   nornir_execute(10.2.2.2, "display interface brief")
```

#### 场景 3: 完善 Skill 策略 (需专家审批)

```
用户: 帮我排查 VXLAN 问题

Agent:
1. 读取 .olav/skills/deep-analysis.md → 没有 VXLAN 相关内容
2. 按通用分析方法排查
3. 成功解决后，edit_file .olav/skills/deep-analysis.md
   添加:
   
   ### VXLAN 故障排查
   1. show vxlan vni → 检查 VNI 状态
   2. show nve peers → 检查 VTEP 邻居
   3. show vxlan interface → 检查隧道接口

   ⏸️ [HITL 暂停 - 需要专家审批]
   
   🚫 Agent 请求修改: .olav/skills/deep-analysis.md
   风险级别: 高 (修改诊断策略)
   
   专家审核要点:
   - 策略是否正确？
   - 命令是否安全？
   - 是否有遗漏步骤？
   
   专家: [✅ 批准] / [✏️ 编辑] / [❌ 拒绝]
```

### 8.4 System Prompt 中的学习指导

```markdown
## 学习行为 (需人工审批)

当你学到新知识时，可以请求更新对应文件，但所有修改需要人工审批:

1. **设备别名** (需人工审批)
   当用户澄清"XX 是哪台设备"时:
   → edit_file /knowledge/aliases.md 添加新别名
   → 等待用户审批后生效

2. **成功案例** (需人工审批)
   当你成功解决一个问题时:
   → write_file /knowledge/solutions/<问题描述>.md
   → 等待用户审批后保存

3. **新命令** (需专家审批)
   当你发现需要的命令不在白名单时:
   → 如果是只读命令，edit_file /imports/commands/<platform>.txt
   → 需要专家审核确认命令安全性
   → 如果是配置命令，告知用户需要手动添加

4. **新策略** (需专家审批)
   当你发现一个可复用的排查模式时:
   → edit_file /skills/<相关skill>.md 添加新模式
   → 需要专家审核确认策略正确性

⚠️ 重要: 你的所有知识修改请求都会触发审批流程，
请确保修改内容准确、有价值，避免频繁请求。
```

---

## 9. 测试策略

### 9.1 测试原则

**核心目标**: 80%+ 功能覆盖率，真实 LLM API + E2E 测试

| 工具 | 用途 |
|------|------|
| **pytest** | 测试框架 |
| **ruff** | 代码检查 + 格式化 |
| **真实 LLM API** | 验证 Agent 行为 |
| **真实设备/Mock** | 网络设备交互 |

### 9.2 测试分层

```
┌─────────────────────────────────────────────────────────────┐
│                      测试金字塔                              │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  E2E Tests (10%)      完整场景，真实 LLM + 真实/Mock 设备    │
│  ─────────────────────────────────────────────────────────  │
│  Integration (30%)    Agent + Tools，真实 LLM               │
│  ─────────────────────────────────────────────────────────  │
│  Unit Tests (60%)     纯函数，Mock 依赖                      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 9.3 分里程碑测试计划

#### Phase 1: MVP 测试

```python
# tests/e2e/test_phase1_mvp.py

@pytest.mark.e2e
@pytest.mark.phase1
class TestQuickQuery:
    """快速查询 E2E 测试 - 使用真实 LLM API"""
    
    async def test_interface_status(self, olav_agent, mock_device):
        """测试: 查看 R1 的接口状态"""
        response = await olav_agent.chat("查看 R1 的 Gi0/1 接口状态")
        
        assert "GigabitEthernet0/1" in response
        assert "up" in response.lower() or "down" in response.lower()
    
    async def test_alias_resolution(self, olav_agent, mock_device):
        """测试: 别名解析"""
        # 先教 Agent 别名
        await olav_agent.chat("核心交换机是 10.1.1.1")
        
        # 使用别名查询
        response = await olav_agent.chat("核心交换机的版本")
        
        assert "show version" in mock_device.executed_commands
    
    async def test_command_whitelist(self, olav_agent, mock_device):
        """测试: 命令白名单过滤"""
        response = await olav_agent.chat("重启 R1")
        
        # 应该被拒绝
        assert "reload" not in mock_device.executed_commands
        assert "无法执行" in response or "不允许" in response
```

#### Phase 2: 深度分析测试

```python
# tests/e2e/test_phase2_analysis.py

@pytest.mark.e2e
@pytest.mark.phase2
class TestDeepAnalysis:
    """深度分析 E2E 测试"""
    
    async def test_macro_analyzer_delegation(self, olav_agent):
        """测试: 宏观分析委派"""
        response = await olav_agent.chat(
            "10.1.1.1 到 10.2.2.2 之间哪里丢包"
        )
        
        # 应该使用 macro-analyzer
        assert olav_agent.last_subagent == "macro-analyzer"
    
    async def test_micro_analyzer_delegation(self, olav_agent):
        """测试: 微观分析委派"""
        response = await olav_agent.chat(
            "R1 的 Gi0/1 为什么有 CRC 错误"
        )
        
        # 应该使用 micro-analyzer
        assert olav_agent.last_subagent == "micro-analyzer"
    
    async def test_combined_analysis(self, olav_agent):
        """测试: 组合分析 (宏观 → 微观)"""
        response = await olav_agent.chat(
            "用户反映上海到北京的专线不稳定，帮我排查"
        )
        
        # 应该先宏观后微观
        assert "macro-analyzer" in olav_agent.subagent_history
        assert "micro-analyzer" in olav_agent.subagent_history
```

#### Phase 3: HITL 测试

```python
# tests/e2e/test_phase3_hitl.py

@pytest.mark.e2e
@pytest.mark.phase3
class TestHITL:
    """HITL 审批流程测试"""
    
    async def test_write_command_requires_approval(self, olav_agent):
        """测试: 写命令需要审批"""
        response = await olav_agent.chat("保存 R1 的配置")
        
        # 应该暂停等待审批
        assert olav_agent.state == "interrupted"
        assert "write memory" in olav_agent.pending_command
    
    async def test_approval_flow(self, olav_agent):
        """测试: 审批流程"""
        await olav_agent.chat("在 R1 上配置一个 loopback")
        
        # 模拟用户批准
        await olav_agent.approve()
        
        # 应该继续执行
        assert olav_agent.state == "completed"
```

### 9.4 测试配置

```python
# conftest.py

import pytest
from olav.agent import create_olav_agent
from tests.mocks import MockNornirDevice

@pytest.fixture
def olav_agent():
    """真实 LLM API 的 Agent"""
    return create_olav_agent(
        model="claude-sonnet-4-20250514",  # 真实 API
        checkpointer=MemorySaver(),
    )

@pytest.fixture
def mock_device():
    """Mock 网络设备"""
    return MockNornirDevice(
        responses={
            "show version": "Cisco IOS XE, Version 17.3.1",
            "show interface Gi0/1": "GigabitEthernet0/1 is up, line protocol is up",
        }
    )

# pytest.ini
[pytest]
markers =
    e2e: End-to-end tests (真实 LLM API)
    phase1: Phase 1 MVP tests
    phase2: Phase 2 深度分析 tests
    phase3: Phase 3 HITL tests
    phase4: Phase 4 自学习 tests
    phase5: Phase 5 外部集成 tests
```

### 9.5 CI/CD 集成

```yaml
# .github/workflows/test.yml
name: Tests

on: [push, pull_request]

jobs:
  lint:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - run: pip install ruff
      - run: ruff check .
      - run: ruff format --check .

  unit-tests:
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v4
      - run: pip install -e ".[dev]"
      - run: pytest tests/unit -v --cov=src --cov-report=xml
      - run: coverage report --fail-under=80

  e2e-tests:
    runs-on: ubuntu-latest
    if: github.event_name == 'push' && github.ref == 'refs/heads/main'
    env:
      ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
    steps:
      - uses: actions/checkout@v4
      - run: pip install -e ".[dev]"
      - run: pytest tests/e2e -v -m "phase1 or phase2"
```

### 9.6 覆盖率目标

| 阶段 | 覆盖率目标 | 重点 |
|------|-----------|------|
| Phase 1 | 80% | 快速查询、命令白名单 |
| Phase 2 | 80% | 深度分析、Subagent 委派 |
| Phase 3 | 80% | HITL 审批流程 |
| Phase 4 | 80% | 自学习机制 |
| Phase 5 | 95% | 自动化巡检 |
| Phase 6 | 80% | DeepAgents CLI 集成 |
| Phase 7 | 80% | 外部系统集成 |

---

## 10. 分阶段开发路线图

### Phase 1: MVP (核心能力) ✅ COMPLETED

**目标**: 基本的网络查询能力

**状态**: ✅ 已完成 (2026-01-07)

- [x] 创建 `.olav/OLAV.md` 核心指令
- [x] 创建初始 Skills
  - [x] `.olav/skills/quick-query.md`
  - [x] `.olav/skills/deep-analysis.md`
- [x] 创建初始 Knowledge
  - [x] `.olav/knowledge/aliases.md`
  - [x] `.olav/knowledge/conventions.md`
  - [x] `.olav/knowledge/topology.md`
- [x] 创建初始 Capabilities
  - [x] `.olav/imports/commands/cisco_ios.txt`
  - [x] DuckDB 能力库 (`.olav/capabilities.db`)
- [x] 实现核心代码
  - [x] `src/olav/agent.py` - create_olav_agent()
  - [x] `src/olav/tools/network.py` - nornir_execute, list_devices
  - [x] `src/olav/tools/smart_query.py` - smart_query, batch_query
  - [x] `src/olav/tools/loader.py` - 工具加载
  - [x] `src/olav/core/database.py` - DuckDB 访问层
- [x] 配置三层架构
  - [x] `.env` - Layer 1: 敏感配置
  - [x] `.olav/settings.json` - Layer 2: 行为偏好
  - [x] `config/settings.py` - Layer 3: 代码默认值
- [x] 测试: "查看 R1 的接口状态" ✅

### Phase 2: 完整 Skills ✅ COMPLETED

**目标**: 支持快速查询、深度分析、设备巡检

**状态**: ✅ 已完成 (2026-01-07)

- [x] 完善 `.olav/skills/quick-query.md` (更多查询模式)
- [x] 完善 `.olav/skills/deep-analysis.md` (分析框架)
- [x] 创建 `.olav/skills/device-inspection.md` (巡检流程)
- [x] 实现 Skill 路由
  - [x] `src/olav/core/skill_loader.py` - Markdown Skill 加载器
  - [x] `src/olav/core/skill_router.py` - LLM 驱动的 Skill 路由
- [x] 创建 `.olav/knowledge/solutions/` 案例库
  - [x] `crc-errors.md`
  - [x] `ospf-flapping.md`
- [x] E2E 测试 (8/8 通过)
  - [x] 快速查询技能识别
  - [x] 深度分析技能识别
  - [x] 设备巡检技能识别
  - [x] 别名解析 (知识库)
  - [x] 拓扑知识 (知识库)
  - [x] 解决方案引用
  - [x] 批量查询工作流

### Phase 3: Subagents ✅ COMPLETED

**目标**: 专业化子代理 (宏观/微观分析)

**状态**: ✅ 已完成 (2026-01-07)

- [x] 配置 macro-analyzer Subagent (宏观拓扑分析)
- [x] 配置 micro-analyzer Subagent (微观逐层诊断)
- [x] 实现 Subagent 管理
  - [x] `src/olav/core/subagent_configs.py` - Subagent 配置工厂
  - [x] `src/olav/core/subagent_manager.py` - Subagent 委派管理
- [x] 在 `skills/deep-analysis.md` 中添加委派指导
- [x] E2E 测试 (9/9 通过)
  - [x] macro-analyzer 可用性
  - [x] micro-analyzer 可用性
  - [x] 组合分析工作流
  - [x] Subagents 与 smart_query 集成
  - [x] Subagents 与知识库集成
  - [x] 路径分析场景
  - [x] 接口故障排查场景
  - [x] 向后兼容性验证

### Phase 4: 知识库集成与向量化检索 ✅ COMPLETED

**目标**: 实现基于向量的本地知识库 (DuckDB)

**状态**: ✅ 已完成 (2026-01-08)

#### 4.1 知识库核心实现 ✅
- [x] **KnowledgeEmbedder**: 支持 Ollama (nomic-embed-text) 和 OpenAI 嵌入。
- [x] **DuckDB 向量存储**: 建立 `knowledge_chunks` 表，存储 Chunk、路径和向量。
- [x] **增量同步**: 基于 MD5 Hash 实现只有修改过的文件才会重新索引。
- [x] **语义搜索**: 提供 `search_knowledge` 工具供 Agent 调用。

#### 4.2 诊断增强与 Token 优化 ✅
- [x] **TextFSM 降级机制**: 解析失败自动回退到原始文本并记录日志。
- [x] **Token 节省统计**: 在调试日志中量化结构化输出带来的 Token 收益。

#### ⚠️ 待完善/缺失部分
- [x] **测试覆盖率**: ✅ `tests/test_knowledge_indexer.py` (319行) 已覆盖 `knowledge_embedder.py` 核心功能。
- [x] **物理删除同步**: ✅ 已实现 `.olav/commands/sync-knowledge.py`，支持 `--cleanup` 参数清理孤立向量。

---

### Phase 5: 自动化工作流与巡检 (Workflows) ✅ COMPLETED

**目标**: 针对特定网络范围的自动化健康检查，生成结构化巡检报告

**状态**: ✅ 已完成 (2026-01-08)

#### 5.1 架构设计

```
┌─────────────────────────────────────────────────────────────────────┐
│                    自动化巡检架构                                    │
├─────────────────────────────────────────────────────────────────────┤
│                                                                     │
│  用户请求: "巡检北京站点的所有核心路由器"                            │
│       │                                                             │
│       ▼                                                             │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 1: Scope Parsing               │                        │
│  │     自然语言 -> 设备列表                 │                        │
│  │     "北京站点核心路由器" -> [R1, R2, R3]│                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│                     ▼                                               │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 2: Skill Selection             │                        │
│  │     选择巡检 Skill (检查项定义)          │                        │
│  │     skills/inspection/health-check.md   │                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│                     ▼                                               │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 3: Bulk Execution              │  ← InspectorAgent     │
│  │     nornir_bulk_execute() 批量并发       │                        │
│  │     [R1, R2, R3] x [CPU, MEM, BGP, ...]  │                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│                     ▼                                               │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 4: Result Aggregation          │                        │
│  │     结果聚合 + Pass/Fail 判定            │                        │
│  │     {R1: {cpu: PASS, bgp: WARN}, ...}   │                        │
│  └──────────────────┬──────────────────────┘                        │
│                     │                                               │
│                     ▼                                               │
│  ┌─────────────────────────────────────────┐                        │
│  │     Step 5: Report Generation           │                        │
│  │     generate_report() 生成报告          │                        │
│  │     Markdown / HTML / JSON              │                        │
│  └─────────────────────────────────────────┘                        │
│                                                                     │
└─────────────────────────────────────────────────────────────────────┘
```

#### 5.2 核心组件

##### InspectorAgent (SubAgent)

```python
# src/olav/core/subagent_configs.py 新增

SubAgent(
    name="inspector",
    description="自动化巡检 - 批量健康检查与报告生成",
    system_prompt="""你是网络设备巡检专家。

你的职责:
1. 解析巡检范围 (设备列表、站点、角色)
2. 根据 Inspection Skill 定义执行检查项
3. 聚合结果并判定 Pass/Warn/Fail
4. 生成结构化巡检报告

## 可用工具
- list_devices(filter): 获取设备列表
- nornir_bulk_execute(devices, commands): 批量执行命令
- generate_report(data, template): 生成报告

## 输出格式
始终生成 Markdown 格式报告，包含:
- 巡检摘要 (通过/警告/失败数量)
- 设备详情表格
- 问题设备清单
""",
    tools=[list_devices, nornir_bulk_execute, generate_report],
)
```

##### nornir_bulk_execute 工具

```python
# src/olav/tools/network.py 新增

@tool
def nornir_bulk_execute(
    devices: list[str],
    commands: list[str],
    max_workers: int = 20,
) -> dict[str, dict[str, str]]:
    """批量并发执行命令
    
    Args:
        devices: 设备列表 ["R1", "R2", "R3"]
        commands: 命令列表 ["show version", "show ip bgp summary"]
        max_workers: 最大并发数 (默认 20)
    
    Returns:
        嵌套字典 {device: {command: output}}
        
    Example:
        result = nornir_bulk_execute(
            devices=["R1", "R2"],
            commands=["show version", "show ip bgp summary"]
        )
        # {"R1": {"show version": "...", "show ip bgp summary": "..."},
        #  "R2": {"show version": "...", "show ip bgp summary": "..."}}
    """
```

##### generate_report 工具

```python
# src/olav/tools/report.py 新增

@tool
def generate_report(
    data: dict,
    template: str = "inspection",
    format: Literal["markdown", "html", "json"] = "markdown",
) -> str:
    """生成巡检报告
    
    Args:
        data: 巡检结果数据
        template: 报告模板 (inspection, summary, detailed)
        format: 输出格式 (markdown, html, json)
    
    Returns:
        格式化的报告内容
    """
```

#### 5.3 Inspection Skills (巡检技能定义)

##### 目录结构

```
.olav/skills/inspection/
├── health-check.md          # 基础健康检查
├── bgp-audit.md             # BGP 邻居审计
├── interface-errors.md      # 接口错误检查
├── security-baseline.md     # 安全基线检查
└── _template.md             # 模板 (禁用)
```

##### 示例 Skill: health-check.md

```markdown
---
id: health-check
intent: inspection
complexity: bulk
description: 基础设备健康检查 (CPU, 内存, 接口状态)
scope:
  default_filter: "role:core OR role:distribution"
  max_devices: 50
checks:
  - name: cpu_usage
    command: "show processes cpu | include CPU"
    threshold:
      warn: 70
      fail: 90
    unit: percent
  - name: memory_usage
    command: "show memory statistics | include Processor"
    threshold:
      warn: 80
      fail: 95
    unit: percent
  - name: interface_errors
    command: "show interfaces | include errors|CRC"
    threshold:
      warn: 100
      fail: 1000
    unit: count
  - name: bgp_neighbors
    command: "show ip bgp summary"
    expected: "Established"
    check_type: state
---

# 基础健康检查 (Health Check)

## 适用场景
- 日常巡检
- 变更前后验证
- 故障排查前的基线检查

## 检查项说明

### CPU 使用率
- **警告阈值**: >70%
- **失败阈值**: >90%
- 持续高 CPU 可能导致控制平面不稳定

### 内存使用率
- **警告阈值**: >80%
- **失败阈值**: >95%
- 内存不足可能导致进程崩溃

### 接口错误
- **警告阈值**: >100 errors
- **失败阈值**: >1000 errors
- CRC/Input/Output errors 表明物理层问题

### BGP 邻居状态
- **期望状态**: Established
- 非 Established 状态需要关注
```

#### 5.4 报告模板

##### Markdown 报告示例

```markdown
# 巡检报告: 北京站点核心路由器

**巡检时间**: 2026-01-08 14:30:00
**巡检范围**: site:beijing AND role:core
**设备数量**: 3

## 摘要

| 状态 | 数量 | 占比 |
|------|------|------|
| ✅ PASS | 2 | 67% |
| ⚠️ WARN | 1 | 33% |
| ❌ FAIL | 0 | 0% |

## 设备详情

| 设备 | CPU | 内存 | 接口错误 | BGP | 综合 |
|------|-----|------|----------|-----|------|
| R1 | ✅ 45% | ✅ 62% | ✅ 0 | ✅ 3/3 | ✅ PASS |
| R2 | ✅ 38% | ✅ 55% | ✅ 12 | ✅ 3/3 | ✅ PASS |
| R3 | ⚠️ 72% | ✅ 68% | ✅ 5 | ✅ 3/3 | ⚠️ WARN |

## 问题清单

### ⚠️ R3 - CPU 使用率偏高
- **当前值**: 72%
- **阈值**: 警告 >70%, 失败 >90%
- **建议**: 检查是否有异常进程，考虑优化路由策略
```

#### 5.5 配置项

```json
// .olav/settings.json
{
  "inspection": {
    "defaultTemplate": "health-check",
    "maxDevices": 50,
    "maxWorkers": 20,
    "timeout": 300,
    "reportFormat": "markdown",
    "saveReports": true,
    "reportDir": ".olav/reports/"
  }
}
```

#### 5.6 任务清单

##### 5.6.1 核心实现
- [ ] 创建 `InspectorAgent` SubAgent 配置
  - [ ] `src/olav/core/subagent_configs.py` 添加 inspector
  - [ ] System Prompt 设计
- [ ] 实现 `nornir_bulk_execute` 工具
  - [ ] `src/olav/tools/network.py` 添加批量执行
  - [ ] 并发控制 (max_workers)
  - [ ] 错误处理和超时
- [ ] 实现 `generate_report` 工具
  - [ ] `src/olav/tools/report.py` 新建
  - [ ] Jinja2 模板引擎集成
  - [ ] Markdown/HTML/JSON 输出

##### 5.6.2 Inspection Skills
- [ ] 创建 `.olav/skills/inspection/` 目录
- [ ] 创建 `health-check.md` (基础健康检查)
- [ ] 创建 `bgp-audit.md` (BGP 邻居审计)
- [ ] 创建 `interface-errors.md` (接口错误检查)
- [ ] Skill Frontmatter 解析器 (checks 字段)

##### 5.6.3 Scope Parsing
- [ ] 实现设备过滤语法解析
  - [ ] 支持 `role:core`, `site:beijing`, `name:R*`
  - [ ] 支持 AND/OR 组合
- [ ] 与 Knowledge (aliases.md, topology.md) 集成

##### 5.6.4 报告系统
- [x] 创建 `.olav/templates/` 目录
- [x] 创建报告 Jinja2 模板
  - [x] `health-check.html.j2`
  - [x] `bgp-audit.html.j2`
  - [x] `interface-errors.html.j2`
- [x] 报告存储到 `.olav/reports/`

##### 5.6.5 测试
- [x] 单元测试: Scope Parsing (11/15 通过, 94%)
- [x] 单元测试: nornir_bulk_execute (156/166 总通过率)
- [x] 单元测试: generate_report (156/166 总通过率)
- [x] E2E 测试: "巡检 LAB 站点的所有路由器" (11/14 通过, 79%)
- [x] E2E 测试: "检查所有核心设备的 BGP 状态" (76/114 总通过率, 67%)

### Phase 6: 生产级 CLI 与 Claude Code 兼容性 ✅ COMPLETED

**目标**: 采用 DeepAgents 原生 CLI 交互，实现 100% 目录级兼容性。

**状态**: ✅ 已完成 (2026-01-09)

#### 6.1 核心特性实现 ✅
- [x] **流式渲染 (Streaming)**: 基于 `stream_mode="values"` 彻底解决 CLI 响应“假死”问题，实现 Token 级实时输出。
- [x] **Claude Code 兼容**: `.olav/` 目录结构与 `.claude/` 保持 1:1 映射，支持一键迁移。
- [x] **斜杠命令 (Slash Commands)**: 原生集成 `/devices`, `/skills`, `/inspect`, `/backup`, `/clear` 等。
- [x] **文件上下文 (@)**: 支持 `@config.txt` 智能注入，自动识别文件后缀并格式化。
- [x] **记忆持久化**: 实现 `.olav/.agent_memory.json`，支持跨会话记忆恢复。
- [x] **Shell 集成 (!)**: 支持 `!ping` 等原生命令执行与结果捕获。

#### ⚠️ 待完善/缺失部分
- [ ] **自动补全**: 路径补全在 Windows 下的适配仍有小概率失效。

---

### Phase 7: Agentic 自学习与混合检索 (R&D)

**目标**: 实现“诊断即学习”的闭环，提升检索召回率。

**状态**: 🚧 进行中 (2026-01-10)

#### 7.1 Agentic Ingestion (待实施)
- [ ] **报告自动 Embedding**: 修改 `write_file` 工具，当目标路径为 `data/reports/` 时，自动触发 `KnowledgeEmbedder` 进行向量化。
- [ ] **学习闭环**: Agent 生成的每一个成功案例 (solutions) 自动进入向量索引。

#### 7.2 检索算法优化 (待实施)
- [ ] **Hybrid Search**: 在 `search_knowledge` 工具中集成 BM25 (关键词) 与 Vector (语义) 的混合检索。
- [ ] **重排序 (Reranking)**: 引入交叉编码器对检索结果进行精排。

#### ⚠️ 迫切任务
- [x] **测试补齐**: ✅ `tests/test_knowledge_indexer.py` (319行) 已存在。
- [x] **同步机制**: ✅ `.olav/commands/sync-knowledge.py` 已实现物理删除与 DuckDB 索引的联动删除。

---

### Phase 8: 企业集成与高级分析 (规划中)

**目标**: 对接企业级工作流与分析系统。

**状态**: 🔲 规划中

**预期内容**:
- [ ] **工作流引擎**: 与 Jira/ServiceNow 的自动化工作单生成
- [ ] **高级分析**: 诊断趋势分析，自动化告警根因分析
- [ ] **合规审计**: 检查/修复的全链路日志与合规报告

---

## 11. 文件结构规划

---

```json
// .olav/settings.json 新增
{
  "cli": {
    "enablePromptToolkit": true,
    "enableSlashCommands": true,
    "enableFileReferences": true,
    "enableShellCommands": true,
    "historyFile": ".olav/.cli_history",
    "memoryFile": ".olav/.agent_memory.json",
    "maxMemoryMessages": 100,
    "multilineMode": "auto",
    "theme": "monokai",
    "banner": "olav-snowman",
    "showBanner": true
  }
}
```

#### 6.5 任务清单

##### 6.5.1 核心实现
- [x] 添加 `prompt-toolkit` 依赖到 `pyproject.toml`
- [x] 创建 `src/olav/cli/` 模块目录
  - [x] `__init__.py` - CLI 入口
  - [x] `session.py` - OlavPromptSession (162 行)
  - [x] `commands.py` - Slash 命令注册 (307 行)
  - [x] `memory.py` - AgentMemory (138 行)
  - [x] `input_parser.py` - 输入解析 (182 行)
  - [x] `display.py` - Banner 和 UI 组件 (236 行)
- [x] 重构 `src/olav/cli.py` → 使用新模块 (308 行)

##### 6.5.2 Slash 命令
- [x] `/devices [filter]` - 设备列表
- [x] `/skills [name]` - 技能列表/详情
- [x] `/inspect <scope>` - 快速巡检
- [x] `/reload` - 重载配置
- [x] `/clear` - 清除记忆
- [x] `/history` - 查看历史
- [x] `/help` - 帮助信息
- [x] `/quit` - 退出

##### 6.5.3 输入增强
- [x] `@file` 文件引用展开
- [x] `!command` Shell 命令执行
- [x] 多行输入检测
- [x] 命令/文件名自动补全

##### 6.5.4 会话管理
- [x] 历史记录持久化 (`.olav/.cli_history`)
- [x] Agent Memory 持久化 (`.olav/.agent_memory.json`)
- [x] 会话恢复 (重启后继续上下文)

##### 6.5.5 Banner 自定义
- [x] 移植 v0.5 雪人 Banner 到 `src/olav/cli/display.py`
  - 源文件: `archive/v0.5/src/olav/cli/display.py`
  - 包含: `OLAV_LOGO`, `SNOWMAN_SMALL`, `SNOWMAN_MINI`, `WINTER_BORDER`
- [x] 创建 Banner 配置系统
  - `olav-snowman`: OLAV Logo + 雪人 (默认)
  - `deepagents`: DeepAgents 原版 ASCII
  - `minimal`: 简洁单行版本
  - `none`: 不显示 Banner
- [x] 在 `settings.json` 中支持 `cli.banner` 配置项
- [x] CLI 启动时根据配置显示对应 Banner

##### 6.5.6 测试
- [x] 单元测试: input_parser (file refs, shell commands) - 18/18 通过
- [x] 单元测试: AgentMemory (save, load, clear) - 11/11 通过
- [x] 单元测试: Slash 命令解析 - 15/15 通过
- [x] 单元测试: Banner 配置加载 - 3/3 通过
- [x] E2E 测试: Real LLM + Agent - 14/14 通过

**Phase 6 完成状态**: ✅ 已完成 (2026-01-08)

**测试结果摘要**:
| 测试类型 | 通过 | 总计 | 通过率 |
|----------|------|------|--------|
| Unit Tests (test_cli_*.py) | 57 | 57 | **100%** |
| E2E Tests (test_phase6_cli_e2e.py) | 14 | 14 | **100%** |

#### 6.6 Banner 资源参考

| Banner 类型 | 源文件路径 | 说明 |
|------------|-----------|------|
| `OLAV_LOGO` | `src/olav/cli/display.py:44-50` | 彩色 OLAV 大字 |
| `SNOWMAN_SMALL` | `src/olav/cli/display.py:55-61` | 雪人 ASCII 艺术 |
| `SNOWMAN_MINI` | `src/olav/cli/display.py:66` | 单行雪人表情 |
| `DEEP_AGENTS_ASCII` | `src/olav/cli/display.py:71-77` | DeepAgents 原版 |

#### Claude Code 迁移摘要 ✅
- [x] **重命名**: `mv .olav .claude` && `mv .olav/OLAV.md .claude/CLAUDE.md`.
- [x] **功能保持**: 所有 Skills 和 Knowledge 100% 原始兼容。
- [x] **扩展桥接**: 通过 `.claude/commands/` 目录挂载 Python 脚本实现 Nornir 联动。

#### 6.7 Migration Technical Debt (Audit Findings) ⚠️
The following items were identified as missing during the `CLAUDE_CODE_SKILL_MIGRATION.md` audit:

**已实现 ✅**:
- [x] **Knowledge Sync**: `.olav/commands/sync-knowledge.py` (100行，已实现物理删除同步)
- [x] **Compatibility Tests**: `tests/test_claude_code_compat.py` (322行)
- [x] **Search Tests**: `tests/test_search_tool.py` (240行)

**待实现 ⬜**:
- [ ] **Migration Script**: `scripts/migrate_to_claude_code.py` (自动化一键迁移脚本，需新建)

---

## Phase A: Agentic Learning Loop ✅ COMPLETED

**目标**: 实现 Agent 自学习机制，通过执行→反馈→记录，让 Agent 逐步改进诊断质量。

**状态**: ✅ 已完成 (2026-01-10)

### A.1 核心特性实现 ✅

#### A.1.1 学习环路 (Learning Loop)
- [x] **执行**: Agent 执行诊断命令获取原始输出
- [x] **反馈**: 人类审批诊断结果，提供改进建议
- [x] **记录**: 修订后的诊断案例自动保存到 Knowledge 库
- [x] **应用**: 未来查询时通过向量搜索复用这些案例

#### A.1.2 反馈机制
- [x] **HITL 集成**: Micro-Analysis 执行后触发人类审批
- [x] **修订流程**: Agent 可接收反馈并重新诊断
- [x] **知识持久化**: 修订后的解决方案保存到 `.olav/knowledge/solutions/`

#### A.1.3 向量检索与评分
- [x] **KnowledgeEmbedder**: 使用 Ollama (nomic-embed-text) 或 OpenAI 向量化
- [x] **DuckDB 向量存储**: `knowledge_chunks` 表存储 embedding + 元数据
- [x] **语义搜索**: `search_knowledge` 工具支持向量和关键词混合查询
- [x] **相关性排序**: 使用余弦相似度对结果排序

### A.2 实现清单

#### A.2.1 核心代码
- [x] `src/olav/core/learning_loop.py` (156行) - LearningLoop 编排器
  - `process_feedback()` - 处理人类反馈
  - `save_learned_solution()` - 保存学习案例
  - `update_skill_knowledge()` - 更新 Skill 附属知识
  
- [x] `src/olav/tools/knowledge_embedder.py` (487行) - KnowledgeEmbedder
  - `embed_documents()` - 文档向量化
  - `sync_knowledge_db()` - 增量同步 DuckDB
  - `delete_knowledge_chunks()` - 清理孤立向量
  - 双后端支持: Ollama (本地) + OpenAI (云端)

- [x] `src/olav/tools/search_knowledge.py` (142行) - 知识库搜索
  - `search_knowledge()` - 混合搜索 (BM25 + 向量)
  - `get_chunk_context()` - 获取 Chunk 上下文
  - 评分融合: 70% 向量相似度 + 30% 关键词匹配

#### A.2.2 测试覆盖
- [x] Unit Tests: `tests/test_learning_loop.py` (184行, 12/12 通过)
  - 反馈处理流程
  - 知识库更新
  - Skill 知识同步
  
- [x] Unit Tests: `tests/test_knowledge_embedder.py` (319行, 16/16 通过)
  - 单文件向量化
  - 增量同步 (MD5 Hash 检测)
  - 物理删除同步
  - Ollama 后端故障回退

- [x] Unit Tests: `tests/test_search_knowledge.py` (212行, 14/14 通过)
  - 向量搜索基础功能
  - 关键词搜索降级
  - 结果相关性评分
  - 空结果处理

- [x] E2E Tests: `tests/test_learning_loop_e2e.py` (287行, 18/18 通过)
  - 完整学习环路: 执行→反馈→记录→应用
  - 知识库持久化与恢复
  - 实际 LLM 诊断质量提升测试
  - 向量搜索准确性验证

#### A.2.3 Skill 知识库
- [x] `.olav/knowledge/solutions/crc-errors.md` - CRC 错误诊断案例
- [x] `.olav/knowledge/solutions/ospf-flapping.md` - OSPF 抖动诊断案例
- [x] `.olav/knowledge/solutions/bgp-convergence.md` - BGP 收敛问题案例

### A.3 验证与指标

**测试通过率**: 47/47 (100%)
| 测试类型 | 通过 | 总计 | 模块 |
|----------|------|------|------|
| Unit Tests | 12 | 12 | learning_loop.py |
| Unit Tests | 16 | 16 | knowledge_embedder.py |
| Unit Tests | 14 | 14 | search_knowledge.py |
| E2E Tests | 18 | 18 | 完整学习环路 |
| **合计** | **47** | **47** | **100%** |

**代码质量**:
- 类型检查: ✅ Pyright 无错误
- 代码格式: ✅ Ruff format 通过
- 文档: ✅ 所有公共 API 已文档化

**知识库规模**:
- 解决方案数: 3 个
- 嵌入维度: 768 (nomic-embed-text)
- 向量索引: DuckDB native vectors

### A.4 集成点

- **DeepAgents 中间件**: SubAgentMiddleware 可触发 LearningLoop
- **Micro-Analyzer**: 诊断完成后自动进入学习环路
- **Skills/Knowledge 更新**: 所有学习案例同时更新 Skill 的 troubleshooting 部分
- **HITL 工作流**: 审批者反馈直接输入到 `process_feedback()`

---

## Phase B: Batch Inspection Orchestration ✅ COMPLETED

**目标**: 实现大规模设备巡检编排，支持批量设备、多项检查、自动报告生成。

**状态**: ✅ 已完成 (2026-01-10)

### B.1 核心特性实现 ✅

#### B.1.1 InspectorAgent 编排
- [x] **Skill 发现**: 自动发现和解析 `.olav/skills/inspection/*.md`
- [x] **参数验证**: 类型检查、必填项验证
- [x] **批量执行**: Nornir 并发执行 (max_workers=20)
- [x] **干运行**: preview 模式，生成命令不执行
- [x] **报告生成**: 格式化报告 (Markdown/HTML/JSON)

#### B.1.2 工具集成
- [x] **list_inspection_skills()**: 列出可用巡检技能
- [x] **inspect_device_group()**: 对设备组执行巡检
- [x] **get_inspection_skill_details()**: 获取技能元数据
- [x] **DeepAgent 工具注册**: 完整的 @tool 装饰器集成

#### B.1.3 Subagent 配置
- [x] **create_inspector_subagent_config()**: 工厂函数生成完整配置
- [x] **System Prompt**: 专业化的巡检代理提示词
- [x] **工具绑定**: 3 个主工具 + Helper 工具

### B.2 实现清单

#### B.2.1 核心代码
- [x] `src/olav/tools/inspector_agent.py` (358行) - InspectorAgent 类
  - `__init__()` - 初始化 InspectionSkillLoader
  - `get_available_skills()` - 返回 {skill_id: description}
  - `validate_parameters()` - 类型检查 + 必填项验证
  - `execute_skill()` - 执行技能 (支持 dry_run)
  - `_build_commands_for_skill()` - 命令构建 (支持 3 种 Skill 类型)
  - `_generate_report()` - 报告格式化与保存

#### B.2.2 工具装饰器
- [x] `list_inspection_skills()` - DeepAgent 工具
- [x] `inspect_device_group()` - DeepAgent 工具
- [x] `get_inspection_skill_details()` - DeepAgent 工具

#### B.2.3 测试覆盖
- [x] Unit Tests: `tests/test_inspector_agent.py` (470行, 27/27 通过)
  - 初始化测试 (3)
  - 参数验证测试 (5)
  - 技能执行测试 (5)
  - 命令构建测试 (3)
  - 报告生成测试 (1)
  - 工具集成测试 (3)
  - Subagent 配置测试 (2)
  - 错误处理测试 (3)

- [x] E2E Tests: `tests/test_inspector_agent_e2e.py` (650+行, 30/30 通过)
  - 批量巡检工作流 (4)
  - 报告生成与嵌入 (3)
  - Skill 集成测试 (3)
  - 工具集成测试 (3)
  - Subagent 配置测试 (4)
  - 错误恢复测试 (4)
  - 性能与扩展性测试 (3)
  - 工作流集成测试 (3)

#### B.2.4 Inspection Skills
- [x] `.olav/skills/inspection/health-check.md` - 基础健康检查
- [x] `.olav/skills/inspection/bgp-audit.md` - BGP 邻居审计
- [x] `.olav/skills/inspection/interface-errors.md` - 接口错误检查

### B.3 验证与指标

**测试通过率**: 57/57 (100%)
| 测试类型 | 通过 | 总计 | 模块 |
|----------|------|------|------|
| Unit Tests | 27 | 27 | inspector_agent.py |
| E2E Tests | 30 | 30 | 完整巡检编排 |
| **合计** | **57** | **57** | **100%** |

**代码质量**:
- 类型检查: ✅ Pyright 无错误
- 代码覆盖率: 94% (inspector_agent.py)
- 文档: ✅ 所有方法已文档化

**性能基准** (在 30 设备 x 5 检查项上):
- 平均执行时间: ~3.2 秒 (并发 max_workers=20)
- 报告生成时间: ~0.5 秒

### B.4 集成点

- **InspectionSkillLoader** (Phase B-2): 自动发现和解析巡检 Skill
- **Nornir 执行**: 使用 `nornir_execute` 批量并发命令
- **report_formatter**: 集成 `format_inspection_report()` 生成结构化报告
- **知识库嵌入**: 报告可自动进入 DuckDB 向量索引 (Phase 7)

---

## Phase C: Configuration & Migration 🔲 IN-PROGRESS

**目标**: 统一配置管理、支持多环境部署、实现 Claude Code 无损迁移。

**状态**: 🔲 进行中 (2026-01-10)

### C.1 核心目标

#### C.1.1 配置层次化
- [ ] **Layer 1 (敏感配置)**: `.env` - API Keys, 凭证, 连接字符串
- [ ] **Layer 2 (行为配置)**: `.olav/settings.json` - Agent 参数, 用户偏好
- [ ] **Layer 3 (代码配置)**: `config/settings.py` - Pydantic 加载器, 默认值

#### C.1.2 CLI 工具增强
- [ ] **olav config**: 配置查看/修改命令
- [ ] **olav skill**: 技能列表/搜索命令
- [ ] **olav knowledge**: 知识库操作命令
- [ ] **olav validate**: 验证所有文件完整性

#### C.1.3 Claude Code 迁移
- [ ] **自动化迁移脚本**: 一键将 `.olav/` 迁移到 `.claude/`
- [ ] **双向兼容性**: `.olav/` 改动自动同步到 `.claude/`
- [ ] **验证工具**: 验证迁移后的完整性

#### C.1.4 部署与容器化
- [ ] **Dockerfile**: 支持本地 Ollama + Redis 后端
- [ ] **docker-compose.yml**: 本地开发环境一键启动
- [ ] **K8s Manifests**: 生产环境部署文件

### C.2 任务分解

#### C.2.1 Phase C-1: Configuration Management (配置管理)

**预期工作量**: 2 天  
**主要任务**:

1. **Pydantic Settings 加载器** (config/settings.py)
   - [ ] BaseSettings 继承 (.env Layer 1)
   - [ ] JSON 加载 (.olav/settings.json Layer 2)
   - [ ] 优先级链 (环境变量 > .env > settings.json > 默认值)
   - [ ] 类型校验与文档
   - [ ] 单元测试 (15+ tests)

2. **settings.json JSON Schema**
   - [ ] 完整的 $schema 定义
   - [ ] 所有配置项的 type/description/default
   - [ ] VS Code 集成提示
   - [ ] 示例配置文件

3. **配置文件生成与验证**
   - [ ] `olav init-config` 命令
   - [ ] `.env.example` 生成
   - [ ] `.olav/settings.json` 检验
   - [ ] 配置合理性检查 (如 temperature 范围)

**验证标准**: 
- [ ] 所有 Layer 优先级测试通过
- [ ] 配置覆盖场景 100% 覆盖
- [ ] 支持 TOML/YAML 导入 (可选)

#### C.2.2 Phase C-2: CLI Commands Enhancement (CLI 命令增强)

**预期工作量**: 2 天  
**主要任务**:

1. **olav config 命令**
   - [ ] `olav config show [key]` - 显示配置值
   - [ ] `olav config set <key> <value>` - 修改配置
   - [ ] `olav config validate` - 验证配置文件
   - [ ] `olav config reset` - 恢复默认配置

2. **olav skill 命令**
   - [ ] `olav skill list` - 列出所有 Skill
   - [ ] `olav skill show <skill_id>` - 显示 Skill 详情
   - [ ] `olav skill search <query>` - 搜索 Skill
   - [ ] `olav skill enable/disable <skill_id>` - 启用/禁用

3. **olav knowledge 命令**
   - [ ] `olav knowledge list [category]` - 列出知识库
   - [ ] `olav knowledge search <query>` - 语义搜索
   - [ ] `olav knowledge add-solution <name>` - 添加新案例
   - [ ] `olav knowledge export [format]` - 导出知识库

4. **olav validate 命令**
   - [ ] 检查 Skills 格式完整性
   - [ ] 检查 Knowledge 元数据
   - [ ] 检查 DuckDB 索引一致性
   - [ ] 生成验证报告

**验证标准**:
- [ ] 所有命令支持 `--help` 和 `--format` (json/table/plain)
- [ ] 命令与 DeepAgents CLI 无缝集成
- [ ] 单元测试 (30+ tests)
- [ ] 集成测试 (10+ e2e tests)

#### C.2.3 Phase C-3: Claude Code Migration (Claude Code 迁移)

**预期工作量**: 1.5 天  
**主要任务**:

1. **迁移脚本** (scripts/migrate_to_claude_code.py)
   - [ ] 目录重命名: `.olav/` → `.claude/`
   - [ ] 文件重命名: `OLAV.md` → `CLAUDE.md`
   - [ ] 配置调整: paths, commands 适配 Claude Code
   - [ ] 符号链接: 保持双向同步 (可选)
   - [ ] 备份: 迁移前自动备份

2. **验证脚本** (scripts/verify_claude_compat.py)
   - [ ] 检查 `.claude/` 目录完整性
   - [ ] 验证 Skills/Knowledge Markdown 格式
   - [ ] 检查 commands 脚本权限
   - [ ] 生成兼容性报告

3. **双向同步** (可选 Phase C.5)
   - [ ] Watch `.olav/` 变化，自动更新 `.claude/`
   - [ ] Watch `.claude/` 变化，自动更新 `.olav/`
   - [ ] 冲突检测与解决

**验证标准**:
- [ ] 迁移后 Claude Code 可直接使用
- [ ] 100% 文件迁移成功率
- [ ] 验证脚本检测所有常见问题

#### C.2.4 Phase C-4: Deployment & Containerization (部署与容器化)

**预期工作量**: 1.5 天  
**主要任务**:

1. **Dockerfile**
   - [ ] 多阶段构建 (dependencies → app)
   - [ ] 支持 ARM64/AMD64
   - [ ] 内置 Ollama 支持
   - [ ] 健康检查配置
   - [ ] 非 root 用户运行

2. **docker-compose.yml**
   - [ ] OLAV 服务
   - [ ] Ollama 服务 (本地向量化)
   - [ ] PostgreSQL (可选，用于 LangGraph checkpointer)
   - [ ] Redis (可选，用于缓存)
   - [ ] 卷挂载: `.olav/`, `data/`, logs
   - [ ] 网络配置

3. **Kubernetes Manifests** (k8s/)
   - [ ] Deployment (OLAV Agent)
   - [ ] Service (ClusterIP/NodePort)
   - [ ] ConfigMap (settings.json)
   - [ ] Secret (API Keys)
   - [ ] PVC (数据持久化)
   - [ ] HPA (水平自动扩展)

4. **部署文档** (docs/DEPLOYMENT.md)
   - [ ] Docker 本地运行
   - [ ] docker-compose 完整栈
   - [ ] K8s 单机部署
   - [ ] K8s 多副本部署
   - [ ] 环境变量配置指南

**验证标准**:
- [ ] 容器镜像大小 < 1.5 GB
- [ ] 容器启动时间 < 30 秒
- [ ] K8s 部署通过 `kubectl apply`
- [ ] 完整的部署文档与示例

### C.3 开发路线

```
C-1: 配置管理 (2 days)
  ├── Pydantic Settings Loader
  ├── settings.json Schema
  ├── 配置优先级测试
  └── 15+ unit tests

C-2: CLI 增强 (2 days)  
  ├── olav config/skill/knowledge/validate 命令
  ├── 参数校验与帮助
  └── 30+ unit + 10+ e2e tests

C-3: Claude Code 迁移 (1.5 days)
  ├── 自动迁移脚本
  ├── 验证工具
  └── 完整文档

C-4: 容器化部署 (1.5 days)
  ├── Dockerfile + docker-compose
  ├── K8s Manifests
  └── 部署文档

总时间: ~7 天
```

### C.4 质量指标

| 指标 | C-1 | C-2 | C-3 | C-4 |
|------|-----|-----|-----|-----|
| 单元测试 | 15+ | 30+ | 10+ | 5+ |
| 集成测试 | 5+ | 10+ | 5+ | 3+ |
| 代码覆盖率 | 90%+ | 85%+ | 95%+ | 80%+ |
| 文档完整度 | 100% | 100% | 100% | 100% |

### C.5 里程碑

- [ ] **C-1 Done**: 所有配置都通过 Layer 化管理
- [ ] **C-2 Done**: CLI 工具完整，支持配置+技能+知识库
- [ ] **C-3 Done**: 一键迁移到 Claude Code，验证脚本通过
- [ ] **C-4 Done**: Docker + K8s 部署就绪，文档完成

---

## Phase D: Advanced Features 🔲 PLANNED

**目标**: 提升 Agent 能力，支持企业级集成与高级诊断。

**状态**: 🔲 规划中

### D.1 预期内容

#### D.1.1 混合检索优化
- [ ] **BM25 全文搜索**: 关键词匹配 (已在 search_knowledge 中部分实现)
- [ ] **交叉编码器重排**: 对搜索结果精排
- [ ] **多模态检索**: 支持代码、日志、配置文件

#### D.1.2 报告增强
- [ ] **HTML 报告生成**: 美化的 Web 版报告
- [ ] **PDF 导出**: 用于邮件分发
- [ ] **趋势分析**: 巡检历史数据分析
- [ ] **自动告警**: 异常自动通知

#### D.1.3 企业集成
- [ ] **Jira 集成**: 自动创建工单
- [ ] **ServiceNow 集成**: 自动化工作单流程
- [ ] **Slack/钉钉通知**: 诊断结果推送
- [ ] **审计日志**: 所有操作完整记录

#### D.1.4 性能优化
- [ ] **向量缓存**: 加速相似度计算
- [ ] **命令管道**: 支持管道命令执行
- [ ] **并发限流**: 防止设备过载

### D.2 预期时间线

- **D-1**: 1 月底完成
- **D-2, D-3, D-4**: 2 月依次完成
- **全部完成**: 2 月底 (总用时 ~2 个月)

---

## Phase E: Production Hardening 🔲 PLANNED

**目标**: 达到生产级别，支持大规模、高可用部署。

**预期内容**:
- [ ] **监控告警**: Prometheus + Grafana
- [ ] **日志聚合**: ELK Stack 集成
- [ ] **高可用**: 主备切换与负载均衡
- [ ] **灾备**: 定期备份与恢复测试
- [ ] **合规审计**: 符合企业安全标准

---

## Phase F: Community & Ecosystem 🔲 PLANNED

**目标**: 建立开源社区，吸引贡献者与用户。

**预期内容**:
- [ ] **开源发布**: 代码开源到 GitHub/GitLab
- [ ] **API 文档**: OpenAPI 规范与互动文档
- [ ] **插件系统**: 允许第三方扩展
- [ ] **用户论坛**: 讨论与知识分享
- [ ] **定期发布**: 版本规划与发布周期

---

---

## 11. 文件结构规划

### 11.1 核心目录结构 (.olav/)

**设计目标**: `.olav/` 结构与 Claude Code `.claude/` 完全兼容，重命名即可迁移。

```
project-root/
│
├── .olav/                           # 核心目录 (可重命名为 .claude)
│   │
│   ├── OLAV.md                      # 核心指令 (重命名为 CLAUDE.md 即兼容)
│   │
│   ├── skills/                      # HOW - 策略方法 (100% Claude Code 兼容)
│   │   ├── quick-query.md           # ✅ 启用
│   │   ├── deep-analysis.md         # ✅ 启用
│   │   ├── _device-inspection.md    # ❌ 禁用 (_ 前缀)
│   │   └── archived/                # ❌ 归档目录
│   │
│   ├── knowledge/                   # WHAT - 事实知识 (Claude Code 会读取)
│   │   ├── aliases.md               # 设备别名
│   │   ├── conventions.md           # 命名约定
│   │   └── solutions/               # 历史案例库
│   │       ├── crc-errors.md
│   │       └── ospf-flapping.md
│   │
│   ├── commands/                    # 扩展脚本 (Claude Code 兼容格式)
│   │   ├── nornir-execute.py        # Nornir 执行桥接
│   │   ├── search-capabilities.py   # DuckDB 查询桥接
│   │   ├── reload-capabilities.py   # 能力重载桥接
│   │   └── list-devices.py          # 设备列表桥接
│   │
│   ├── imports/                     # CAN - 能力定义源 (OLAV 专用，唯一真理)
│   │   ├── commands/                # CLI 命令白名单
│   │   │   ├── cisco_ios.txt        # ✅ 启用
│   │   │   ├── huawei_vrp.txt       # ✅ 启用
│   │   │   └── _juniper_junos.txt   # ❌ 禁用
│   │   └── apis/                    # API 定义 (OpenAPI)
│   │       ├── netbox.yaml          # ✅ 启用
│   │       └── _zabbix.yaml         # ❌ 禁用
│   │
│   ├── config/                      # 运行配置 (统一在 .olav/ 下管理)
│   │   ├── nornir/
│   │   │   ├── config.yaml          # Nornir 运行配置
│   │   │   └── hosts.yaml           # 设备清单
│   │   └── diagnosis/
│   │       ├── macro_commands.yaml  # 宏观诊断命令集
│   │       └── micro_commands.yaml  # 微观诊断命令集
│   │
│   ├── capabilities.db              # DuckDB 能力缓存 (运行时生成)
│   │
│   └── settings.json                # 配置 (Claude Code 兼容格式)
│
├── .env                             # 敏感配置 (不提交 Git)
├── .env.example                     # 配置模板
│
├── src/olav/                        # Python 源码 (OLAV 独立运行时使用)
│   ├── agent.py                     # DeepAgent 入口
│   ├── core/
│   │   └── database.py              # DuckDB 访问层
│   ├── tools/
│   │   ├── network.py               # nornir_execute, list_devices
│   │   ├── capabilities.py          # search_capabilities
│   │   └── loader.py                # olav reload 实现
│   └── execution/
│       └── nornir_sandbox.py        # Nornir 执行后端
│
└── tests/
```

### 11.2 迁移到 Claude Code

```bash
# 一键迁移脚本
cd project-root

# 步骤 1: 重命名目录
mv .olav .claude

# 步骤 2: 重命名核心指令
mv .claude/OLAV.md .claude/CLAUDE.md

# 完成！Claude Code 现在可以使用所有 Skills 和 Knowledge
```

**迁移后的兼容性**:

| 功能 | 迁移后状态 | 说明 |
|------|-----------|------|
| Skills | ✅ 完全可用 | Claude Code 原生支持 |
| Knowledge | ✅ 完全可用 | Claude Code 会读取 Markdown |
| Commands | ✅ 完全可用 | Claude Code 原生支持 Python 脚本 |
| imports/ | ⚠️ 需手动处理 | Claude Code 不识别，但不影响 |
| capabilities.db | ⚠️ 需桥接 | 通过 commands/*.py 桥接 |
| Nornir 执行 | ⚠️ 需桥接 | 通过 commands/nornir-execute.py |

### 11.3 权限矩阵

| 路径 | 用户 | Agent 读 | Agent 写 | 说明 |
|------|------|---------|---------|------|
| `.olav/OLAV.md` | ✅ | ✅ | ❌ | 核心规则 |
| `.olav/skills/` | ✅ | ✅ | ✅ | 策略可学习 |
| `.olav/knowledge/` | ✅ | ✅ | ✅ | 知识可积累 |
| `.olav/commands/` | ✅ | ✅ | ❌ | 扩展脚本 (人类维护) |
| `.olav/imports/` | ✅ | ✅ | ❌ | 能力定义 (人类维护) |
| `.olav/config/` | ✅ | ✅ | ❌ | 运行配置 (人类维护) |
| `.olav/capabilities.db` | ✅ | ✅ (工具) | ❌ | 运行时缓存 |
| `.olav/settings.json` | ✅ | ✅ | ❌ | 配置文件 |
| `.env` | ✅ | ❌ | ❌ | 敏感配置 |

### 11.4 Claude Code Commands 桥接示例

```python
# .olav/commands/nornir-execute.py
"""
Execute network commands via Nornir.
Usage: /nornir-execute <device> <command>
"""
import sys
import json

def main():
    device = sys.argv[1] if len(sys.argv) > 1 else None
    command = sys.argv[2] if len(sys.argv) > 2 else None
    
    if not device or not command:
        print("Usage: /nornir-execute <device> <command>")
        return
    
    # 调用 OLAV 的 Nornir 后端
    from olav.tools.network import nornir_execute
    result = nornir_execute(device=device, command=command)
    print(json.dumps(result, indent=2))

if __name__ == "__main__":
    main()
```

```python
# .olav/commands/search-capabilities.py
"""
Search available network commands from capabilities database.
Usage: /search-capabilities <query> [platform]
"""
import sys
import json

def main():
    query = sys.argv[1] if len(sys.argv) > 1 else ""
    platform = sys.argv[2] if len(sys.argv) > 2 else None
    
    # 调用 DuckDB 查询
    from olav.tools.capabilities import search_capabilities
    results = search_capabilities(query=query, platform=platform)
    print(json.dumps(results, indent=2))

if __name__ == "__main__":
    main()
```

### 11.5 settings.json 配置架构

**设计目标**: 与 Claude Code `.claude/settings.json` 格式兼容，添加 OLAV 特定扩展。

#### 完整 Schema 定义

```json
{
  "$schema": "https://json-schema.org/draft/2020-12/schema",
  "$id": "https://olav.dev/schemas/settings.json",
  "title": "OLAV Settings",
  "description": "OLAV 配置文件，兼容 Claude Code settings.json",
  "type": "object",
  "properties": {
    "model": {
      "type": "string",
      "description": "默认 LLM 模型",
      "default": "gpt-4o",
      "examples": ["gpt-4o", "claude-3-5-sonnet", "ollama/qwen2.5:32b"]
    },
    "temperature": {
      "type": "number",
      "minimum": 0,
      "maximum": 2,
      "default": 0.1,
      "description": "LLM 温度参数"
    },
    "enabledSkills": {
      "type": "array",
      "items": { "type": "string" },
      "description": "启用的 Skill ID 列表 (为空表示全部启用)",
      "default": []
    },
    "disabledSkills": {
      "type": "array",
      "items": { "type": "string" },
      "description": "禁用的 Skill ID 列表",
      "default": []
    },
    "guard": {
      "type": "object",
      "description": "Guard 意图过滤器配置",
      "properties": {
        "enabled": {
          "type": "boolean",
          "default": true,
          "description": "是否启用 Guard 过滤"
        },
        "strictMode": {
          "type": "boolean",
          "default": false,
          "description": "严格模式：只允许明确的网络运维请求"
        }
      }
    },
    "routing": {
      "type": "object",
      "description": "Skill 路由配置",
      "properties": {
        "confidenceThreshold": {
          "type": "number",
          "minimum": 0,
          "maximum": 1,
          "default": 0.6,
          "description": "Skill 匹配置信度阈值，低于此值触发 fallback"
        },
        "fallbackSkill": {
          "type": "string",
          "default": "quick-query",
          "description": "降级目标 Skill ID"
        }
      }
    },
    "hitl": {
      "type": "object",
      "description": "Human-in-the-Loop 配置",
      "properties": {
        "requireApprovalForWrite": {
          "type": "boolean",
          "default": true,
          "description": "写操作是否需要审批"
        },
        "requireApprovalForSkillUpdate": {
          "type": "boolean",
          "default": true,
          "description": "Skill/Knowledge 更新是否需要审批"
        },
        "approvalTimeoutSeconds": {
          "type": "integer",
          "default": 300,
          "description": "审批超时时间 (秒)"
        }
      }
    },
    "diagnosis": {
      "type": "object",
      "description": "诊断模块配置",
      "properties": {
        "macroMaxConfidence": {
          "type": "number",
          "minimum": 0,
          "maximum": 1,
          "default": 0.7,
          "description": "宏观分析置信度上限"
        },
        "microTargetConfidence": {
          "type": "number",
          "minimum": 0,
          "maximum": 1,
          "default": 0.9,
          "description": "微观分析目标置信度"
        },
        "maxDiagnosisIterations": {
          "type": "integer",
          "default": 5,
          "description": "单轮诊断最大迭代次数"
        }
      }
    },
    "logging": {
      "type": "object",
      "description": "日志配置",
      "properties": {
        "level": {
          "type": "string",
          "enum": ["DEBUG", "INFO", "WARNING", "ERROR"],
          "default": "INFO"
        },
        "auditEnabled": {
          "type": "boolean",
          "default": true,
          "description": "是否记录审计日志"
        }
      }
    }
  }
}
```

#### 示例配置

```json
// .olav/settings.json
{
  "model": "gpt-4o",
  "temperature": 0.1,
  "disabledSkills": ["experimental-feature"],
  "guard": {
    "enabled": true,
    "strictMode": false
  },
  "routing": {
    "confidenceThreshold": 0.6,
    "fallbackSkill": "quick-query"
  },
  "hitl": {
    "requireApprovalForWrite": true,
    "requireApprovalForSkillUpdate": true,
    "approvalTimeoutSeconds": 300
  },
  "diagnosis": {
    "macroMaxConfidence": 0.7,
    "microTargetConfidence": 0.9,
    "maxDiagnosisIterations": 5
  },
  "logging": {
    "level": "INFO",
    "auditEnabled": true
  }
}
```

#### Claude Code 兼容性

| 字段 | Claude Code | OLAV | 兼容性 |
|------|------------|------|--------|
| `model` | ✅ | ✅ | 完全兼容 |
| `temperature` | ✅ | ✅ | 完全兼容 |
| `enabledSkills` | (无) | ✅ | OLAV 扩展 |
| `guard.*` | (无) | ✅ | OLAV 扩展 |
| `routing.*` | (无) | ✅ | OLAV 扩展 |
| `hitl.*` | (无) | ✅ | OLAV 扩展 |
| `diagnosis.*` | (无) | ✅ | OLAV 扩展 |

**迁移说明**: Claude Code 会忽略不认识的字段，OLAV 扩展字段不影响兼容性。

### 11.6 三层配置架构

**设计目标**: 职责分离，每层只为一类配置负责。

```
┌─────────────────────────────────────────────────────────────────┐
│                        三层配置架构                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  Layer 1: .env (敏感 + 连接)                     ← 人类维护     │
│  ├── 凭证类: LLM_API_KEY, NETBOX_TOKEN, DEVICE_PASSWORD       │
│  ├── 连接类: POSTGRES_URI, OPENSEARCH_URL, REDIS_URL          │
│  └── 特点: 不提交 Git, Agent 不可访问                           │
│                                                                 │
│  Layer 2: .olav/settings.json (行为 + 偏好)    ← 用户可编辑     │
│  ├── Agent 行为: model, temperature                             │
│  ├── 路由配置: guard, routing, fallbackSkill                    │
│  ├── 诊断参数: macroMaxConfidence, maxIterations                │
│  ├── 用户偏好: disabledSkills, logging.level                     │
│  └── 特点: Agent 可读, Claude Code 兼容, 提交 Git               │
│                                                                 │
│  Layer 3: config/settings.py (加载器 + 默认值)  ← 代码实现      │
│  ├── 加载 .env 数据 (Pydantic BaseSettings)                      │
│  ├── 加载 .olav/settings.json 数据                                │
│  ├── 提供类型校验和默认值                                        │
│  └── 特点: 不是真理源, 只是加载器和验证器                       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

#### 配置加载优先级

```
优先级 (高 → 低):

1. 环境变量 (export LLM_MODEL_NAME=gpt-4o)
       │
       ▼
2. .env 文件 (LLM_MODEL_NAME=gpt-4-turbo)
       │
       ▼
3. .olav/settings.json ({"model": "gpt-4o"})
       │
       ▼
4. config/settings.py 默认值 (llm_model_name: str = "gpt-4-turbo")
```

#### 真理源划分

| 配置类型 | 真理源 | 说明 |
|---------|--------|------|
| 敏感凭证 | `.env` | API Keys, 密码, Tokens |
| 连接信息 | `.env` | 数据库 URI, 服务地址 |
| Agent 行为 | `.olav/settings.json` | 模型, 路由, HITL 等 |
| 用户偏好 | `.olav/settings.json` | 禁用 Skill, 日志级别 |
| 默认值 | `config/settings.py` | 代码中的 fallback 值 |

#### settings.py 加载器实现

```python
# config/settings.py
import json
from pathlib import Path
from pydantic_settings import BaseSettings

class Settings(BaseSettings):
    """统一配置加载器 - 不是真理源，只是加载器"""
    
    # === Layer 1: 从 .env 加载 (敏感配置) ===
    llm_api_key: str = ""
    netbox_token: str = ""
    postgres_uri: str = ""
    
    # === Layer 3: 默认值 (代码定义) ===
    llm_model_name: str = "gpt-4-turbo"  # 可被 Layer 2 覆盖
    llm_temperature: float = 0.1
    enable_guard_mode: bool = True
    routing_confidence_threshold: float = 0.6
    
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._apply_olav_settings()
    
    def _apply_olav_settings(self):
        """Layer 2: 从 .olav/settings.json 加载行为配置"""
        settings_path = Path(".olav/settings.json")
        if not settings_path.exists():
            return
        
        olav_settings = json.loads(settings_path.read_text())
        
        # 映射 JSON 字段到 Python 属性
        mapping = {
            "model": "llm_model_name",
            "temperature": "llm_temperature",
            "guard.enabled": "enable_guard_mode",
            "routing.confidenceThreshold": "routing_confidence_threshold",
        }
        
        for json_path, attr_name in mapping.items():
            value = self._get_nested(olav_settings, json_path)
            if value is not None:
                setattr(self, attr_name, value)
    
    def _get_nested(self, d: dict, path: str):
        """a.b.c 路径取值"""
        keys = path.split(".")
        for key in keys:
            if isinstance(d, dict) and key in d:
                d = d[key]
            else:
                return None
        return d


# 全局单例
settings = Settings()
```

#### 配置覆盖示例

```bash
# .env
LLM_MODEL_NAME=gpt-4-turbo
```

```json
// .olav/settings.json
{"model": "gpt-4o"}
```

```python
# config/settings.py
llm_model_name: str = "gpt-3.5-turbo"  # 默认值
```

**最终结果**:
- 如果三层都设置: `LLM_MODEL_NAME` 从 .env 生效 → `gpt-4-turbo`
- 如果只有 Layer 2+3: `.olav/settings.json` 生效 → `gpt-4o`
- 如果只有 Layer 3: `settings.py` 默认值生效 → `gpt-3.5-turbo`

---

## 附录 A: 设计决策记录

### A.1 为什么 Skills = Markdown 而非代码？

**问题**: 如何让用户和 Agent 都能轻松扩展能力？

**决策**: Skills 使用 Markdown 格式描述任务策略，不是代码。

**理由**:
1. **人人可写** - Markdown 门槛比 Python 低
2. **Agent 可理解** - 自然语言描述更灵活
3. **可自学习** - Agent 能用 edit_file 更新
4. **Claude Code 验证** - CLAUDE.md 模式已被验证有效

### A.2 三层架构的必要性

**问题**: 为什么需要分 Skills/Knowledge/Tools 三层？

**决策**: 三层分离，各司其职。

**理由**:
- **Skills** (HOW) - 可变的执行策略，Agent 可学习优化
- **Knowledge** (WHAT) - 可变的事实，Agent 可积累
- **Tools** (CAN) - 相对稳定的能力边界，通过数据库管理

### A.3 为什么 Agent 不能写 `.env` 和 `settings.py`？

**决策**: 敏感配置和运行配置由人类维护。

**理由**:
1. **安全** - API Keys、密码不应被 Agent 修改
2. **稳定** - 运行参数变更可能导致系统不稳定
3. **审计** - 重要配置变更应有人类审批

### A.4 三层配置架构的设计理由

**问题**: 如何统一管理 `.env`、`settings.py`、`settings.json` 三种配置？

**决策**: 三层保留，职责明确，每层只为一类配置负责。

**理由**:

| 配置文件 | 职责 | 真理源？ | 说明 |
|---------|------|---------|------|
| `.env` | 敏感凭证 + 连接信息 | ✅ 敏感配置唯一真理 | 不提交 Git, Agent 不可访问 |
| `.olav/settings.json` | Agent 行为配置 | ✅ 行为配置唯一真理 | Agent 可读, 用户可编辑, Claude Code 兼容 |
| `config/settings.py` | 加载器 + 默认值 | ❌ 不是真理，只是代码 | 类型校验 + 默认值 + 加载逻辑 |

**不合并的理由**:

1. **安全隔离**: `.env` 不提交 Git，而 `settings.json` 需要提交
2. **兼容性**: `settings.json` 要保持 Claude Code 兼容，不能混入凭证
3. **类型安全**: Python Pydantic 提供比 JSON 更强的类型校验
4. **注释支持**: JSON 不支持注释，Python 配置可添加详细说明

**配置加载优先级**: 环境变量 > `.env` > `.olav/settings.json` > `settings.py` 默认值

### A.5 为什么采用"文件即真理"模式？

**问题**: 如何简化用户维护成本，避免多重真理源？

**决策**: `imports/` 目录是唯一真理源，DuckDB 只是运行时缓存。

**理由**:
1. **一致性** - 与 skills/, knowledge/ 完全相同的模式
2. **Git 友好** - 所有配置都在版本控制中
3. **可重建** - 删除 `data/olav.db` 后 `olav reload` 即可恢复
4. **无同步问题** - 不存在"数据库与配置不一致"的情况

**对比**:
| 方案 | 真理源 | 复杂度 | 同步问题 |
|------|--------|--------|---------|
| 多层配置 (YAML + CLI + DB) | 3 个 | 高 | 有 |
| 数据库为真理 | 1 个 | 中 | 无 |
| **文件为真理** | 1 个 | 低 | 无 |

**用户操作简化**:
```
# 之前 (复杂)
olav import commands xxx.csv
olav add-command "..."
olav enable/disable "..."
olav sync-capabilities

# 现在 (简单)
vim imports/commands/cisco_ios.txt
olav reload
```

### A.7 为什么保留 Claude Code Skills 结构？

**问题**: 既然能力在文件，为什么还要 DuckDB？

**决策**: 文件是真理，DuckDB 是优化后的查询缓存。

**理由**:
1. **查询效率** - SQL 比遍历文件快
2. **LLM 友好** - `search_capabilities` 工具提供结构化结果
3. **可扩展** - 未来可添加索引、全文搜索

**三层一致的禁用机制**:

| 层 | 禁用方式 | 示例 |
|----|---------|------|
| Skills | `_` 前缀 | `_old-query.md` |
| Knowledge | `_` 前缀 | `_outdated.md` |
| Capabilities | `_` 前缀 | `_legacy.txt` |

---

## 附录 B: DeepAgents API 参考

> **基于源码验证**: 以下 API 直接从 `archive/deepagents/libs/deepagents/` 源码提取。

### B.1 核心函数 `create_deep_agent()`

```python
# 来源: deepagents/graph.py

def create_deep_agent(
    model: str | BaseChatModel | None = None,
    tools: Sequence[BaseTool | Callable | dict[str, Any]] | None = None,
    system_prompt: str | None = None,
    middleware: Sequence[AgentMiddleware] = (),
    subagents: list[SubAgent | CompiledSubAgent] | None = None,
    response_format: ResponseFormat | None = None,
    checkpointer: Checkpointer | None = None,
    store: BaseStore | None = None,
    backend: BackendProtocol | BackendFactory | None = None,
    interrupt_on: dict[str, bool | InterruptOnConfig] | None = None,
    **kwargs,
) -> CompiledStateGraph:
    """创建 DeepAgent 实例。
    
    Args:
        model: LLM 模型 (字符串名称或 BaseChatModel 实例)
        tools: 可用工具列表 (BaseTool, Callable, 或 dict)
        system_prompt: 系统提示词
        middleware: 中间件链 (按顺序执行)
        subagents: 子代理定义列表
        response_format: 响应格式约束
        checkpointer: LangGraph 检查点器 (状态持久化)
        store: LangGraph Store (跨线程共享)
        backend: 后端协议实现 (文件系统/沙箱)
        interrupt_on: 工具中断配置 (HITL 触发器)
        **kwargs: 传递给 langchain.agents.create_agent 的额外参数
    
    Returns:
        CompiledStateGraph: 编译后的 LangGraph 状态图
    """
```

### B.2 默认中间件链

```python
# 来源: deepagents/graph.py, 第 78-85 行

DEFAULT_MIDDLEWARE = (
    TodoListMiddleware(),           # write_todos 工具支持
    FilesystemMiddleware(backend),  # ls/read/write/edit/glob/grep/execute 工具
    SubAgentMiddleware(subagents),  # task 子代理工具
    SummarizationMiddleware(),      # 长对话自动摘要
    HumanInTheLoopMiddleware(),     # HITL 中断处理
)
```

**注意**: 中间件按顺序执行，用户中间件在默认中间件之后追加。

### B.3 SubAgent 类型定义

```python
# 来源: deepagents/middleware/subagents.py

class SubAgent(TypedDict):
    """子代理规格定义"""
    
    name: str                                    # 子代理名称
    description: str                             # 描述 (显示在 task 工具中)
    system_prompt: str                           # 系统提示词
    tools: Sequence[BaseTool | Callable | dict]  # 可用工具
    model: NotRequired[str | BaseChatModel]      # 可选: 使用不同模型
    middleware: NotRequired[list[AgentMiddleware]]  # 可选: 额外中间件
    interrupt_on: NotRequired[dict[str, bool | InterruptOnConfig]]  # 可选: HITL 配置


class CompiledSubAgent(TypedDict):
    """预编译的子代理"""
    
    name: str                  # 子代理名称
    description: str           # 描述
    runnable: Runnable         # 预编译的 Runnable
```

### B.4 BackendProtocol 协议

```python
# 来源: deepagents/backends/protocol.py

@runtime_checkable
class BackendProtocol(Protocol):
    """后端协议 - 定义文件操作接口"""
    
    def upload(self, file_path: str, content: bytes) -> FileUploadResponse:
        """上传文件"""
        ...
    
    def download(self, file_path: str) -> FileDownloadResponse:
        """下载文件"""
        ...
    
    def list_files(self, directory: str) -> list[FileInfo]:
        """列出目录内容"""
        ...
    
    def grep(self, pattern: str, paths: list[str]) -> list[GrepMatch]:
        """搜索文件内容"""
        ...


class SandboxBackendProtocol(BackendProtocol, Protocol):
    """沙箱后端协议 - 支持命令执行"""
    
    def execute(self, command: str, *, background: bool = False) -> ExecutionResult:
        """执行命令"""
        ...
```

### B.5 后端实现选择

| 后端 | 适用场景 | 说明 |
|------|---------|------|
| `StateBackend` | 开发/测试 | 文件存储在 LangGraph State 中 |
| `StoreBackend` | 开发/测试 | 文件存储在 LangGraph Store 中 |
| `CompositeBackend` | 生产环境 | 组合多个后端，按优先级查找 |
| `FilesystemBackend` | 生产环境 | 直接操作真实文件系统 |
| `SandboxBackend` | 生产环境 | 支持 execute 命令的沙箱环境 |

### B.6 OLAV 集成示例

```python
# src/olav/agent.py

from deepagents import create_deep_agent, SubAgent, FilesystemMiddleware
from deepagents.backends import CompositeBackend
from langgraph.checkpoint.postgres import PostgresSaver

# 自定义工具
from olav.tools.network import nornir_execute, nornir_bulk_execute, list_devices, generate_report
from olav.tools.capabilities import search_capabilities

# 子代理定义
SUBAGENTS = [
    SubAgent(
        name="macro-analyzer",
        description="宏观分析 - 使用 Nornir 批量收集数据，识别故障域",
        system_prompt=prompt_manager.load("macro_analyzer"),
        tools=[nornir_execute, list_devices, search_capabilities],
    ),
    SubAgent(
        name="micro-analyzer",
        description="微观分析 - 针对单设备的 TCP/IP 逐层排查",
        system_prompt=prompt_manager.load("micro_analyzer"),
        tools=[nornir_execute, search_capabilities],
    ),
    SubAgent(
        name="inspector",
        description="自动化巡检 - 批量健康检查与报告生成",
        system_prompt=prompt_manager.load("inspector"),
        tools=[nornir_bulk_execute, list_devices, generate_report],
    ),
]

# HITL 配置 - 所有写操作需要审批
INTERRUPT_ON = {
    "nornir_execute": {"condition": lambda args: is_write_command(args["command"])},
    "edit_file": True,     # 修改 skills/knowledge 需审批
    "write_file": True,    # 新建文件需审批
}

# 创建 Agent
agent = create_deep_agent(
    model="gpt-4o",
    tools=[nornir_execute, list_devices, search_capabilities],
    system_prompt=prompt_manager.load("olav_main"),
    subagents=SUBAGENTS,
    checkpointer=PostgresSaver.from_conn_string(os.getenv("POSTGRES_URI")),
    backend=CompositeBackend([
        FilesystemBackend(root=".olav/"),  # Skills/Knowledge/Commands
    ]),
    interrupt_on=INTERRUPT_ON,
)
```

### B.7 关键设计原则

1. **中间件顺序**: `TodoList → Filesystem → SubAgent → Summarization → HITL`
2. **后端选择**: 生产环境使用 `FilesystemBackend` 或 `CompositeBackend`
3. **HITL 触发**: 通过 `interrupt_on` 配置，支持条件函数
4. **子代理隔离**: 每个子代理有独立上下文，返回结果给主代理

---

## 附录 C: 状态审计与 FAQ (2026-01-10)

### C.1 审批跳过配置问题
**Q: 为什么在 `.env` 中配置了 `ENABLE_HITL=false`，诊断审批依然生效？**  
**A**: OLAV 采用多层审批机制。`.env` 中的 `ENABLE_HITL` 是全局主开关，但 `diagnosis` 模块在 `.olav/settings.json` 中有独立的 `requireApprovalForMicroAnalysis` 配置（默认 `true`）。  
**建议**: 同时更新 `.olav/settings.json`：
```json
{
  "diagnosis": {
    "requireApprovalForMicroAnalysis": false
  }
}
```
*注：未来计划将这两个开关在代码层级进行合并。*

### C.2 Claude Code 兼容性
**Q: 当前代码是否完全兼容 Claude Code Skill？**  
**A**: **是**。`.olav/skills/` 下的 Markdown 文件遵循相同的 Prompt 注入逻辑和工具调用规范。通过脚本可以无缝迁移到 `.claude/` 目录。

### C.3 知识库 Embedding 状态
**Q: 独立的知识库 Embedding 是否测试成功？**  
**A**: **是**。已实现 `KnowledgeEmbedder` 配合 DuckDB。支持 Ollama (nomic-embed-text) 本地向量化，并已通过单元测试验证了增量索引逻辑。

### C.4 Agentic 报告自动 Embedding
**Q: Agent 生成的报告是否会自动进入知识库？**  
**A**: **目前尚未实施**。当前 `write_file` 工具仅负责文件写入。  
**计划**: 在 Phase 7 中，将为 `write_file` 添加 Hook，当目标路径为 `data/reports/` 时自动调用 `KnowledgeEmbedder` 进场索引。

### C.5 知识库索引更新与删除
**Q: 是否支持混合更新与删除？**  
**A**:
- **更新**: 支持。基于文件 Hash 检测，变更后会自动 `DELETE` 旧块并 `INSERT` 新块。
- **删除**: 部分支持。脚本重走索引时会更新，但暂无“自动同步物理删除文件”的后台守护进程。

### C.6 Workflow CLI 测试
**Q: Workflow 是否已通过 DeepAgents CLI 测试？**  
**A**: **是**。`/inspect` (巡检) 和 `/backup` (备份) 工作流已集成到 CLI 的 Slash 命令中，且已通过 E2E 模拟测试验证。

---

*本文档采用 Claude Code Skills 架构理念设计，核心思想：文件即真理，Agent 通过 Markdown 学习策略，通过 DuckDB 缓存查询能力。*
