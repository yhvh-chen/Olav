...
### 🧠 Agentic 自主系统
基于 **DeepAgents** 框架构建，OLAV 不仅仅是一个聊天机器人。它是一个目标导向的智能体，可以：
- **规划**: 将复杂的请求（如"审计所有 BGP 邻居"）分解为子任务。
- **执行**: 自主执行多步骤工作流程，使用工具并检查结果。
- **反思**: 在向用户报告前验证自己的输出。
- **记忆**: 在故障排除会话中保持上下文。
...
- **令牌高效**: 这种 schema 感知的方法可将上下文用量减少 50% 以上，并通过允许 AI 查询特定字段（如 `interface[0].status`）而不是解析文本，显著提高分析精度。
...
---
name: daily-check
description: 对边界路由器执行日常健康检查
version: 1.0
---
# 流程
1. 对"edge"组执行 `/inspect` 工作流。
2. 检查"高 CPU"日志模式。
3. 报告异常。
```
