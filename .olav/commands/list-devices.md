...
type: command
platform: all
description: List available network devices from inventory with optional filtering
---

# List Devices Command

List available network devices from the inventory with optional filtering by role, site, or platform.

## Usage

```
/list-devices [--role <role>] [--site <site>] [--platform <platform>]
```

## Options

- **--role, -r**: Filter by device role (core, access, distribution)
- **--site, -s**: Filter by site name
- **--platform, -p**: Filter by platform (cisco_ios, huawei_vrp)
