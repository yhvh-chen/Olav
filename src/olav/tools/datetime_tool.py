...
async def parse_time_range(natural_text: str, utc_format: bool = True) -> dict[str, Any]:
    """Parse natural language time range to ISO8601 start/end tuple."""
    impl = ToolRegistry.get_tool("parse_time_range")
    if impl is None:
        return {"success": False, "error": "parse_time_range tool not registered"}
    result = await impl.execute(natural_text=natural_text, utc_format=utc_format)
    return {
