...
class QueryDiagnosticWorkflow(BaseWorkflow):
    ...
    async def validate_input(self, user_query: str) -> tuple[bool, str]:
        ...
        # 排除配置变更关键词（更全面）
        config_keywords = [
            "修改", "配置", "设置", "添加", "删除", "shutdown", "no shutdown",
            "change", "modify", "set", "add", "delete", "configure", "config", "edit",
            "commit", "rollback", "create", "remove"
        ]
        ...
        # 匹配查询/诊断关键词
        query_keywords = [
            "查询", "显示", "状态", "为什么", "原因", "诊断", "排查", "分析",
            "show", "query", "why", "cause", "diagnose", "analyze", "check",
            "bgp", "ospf", "route", "interface", "接口", "路由"
        ]
...
"""
        ...
        def route_after_evaluation(state: QueryDiagnosticState) -> Literal["micro_diagnosis", "final_answer"]:
...
__all__ = ["QueryDiagnosticWorkflow", "QueryDiagnosticState"]
