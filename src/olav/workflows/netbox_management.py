...
class NetBoxManagementWorkflow(BaseWorkflow):
    ...
    async def validate_input(self, user_query: str) -> tuple[bool, str]:
        ...
        # 检查是否包含 NetBox 实体类型
        entity_types = ["device", "interface", "ip-address", "site", "rack", "cable", "virtual-machine"]
...
"""
        ...
        def route_after_planning(state: NetBoxManagementState) -> Literal["hitl_approval", "api_execution"]:
        ...
        def route_after_approval(state: NetBoxManagementState) -> Literal["api_execution", "final_answer"]:
...
__all__ = ["NetBoxManagementWorkflow", "NetBoxManagementState"]
