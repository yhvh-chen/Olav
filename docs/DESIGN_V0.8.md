...
#### ⚠️ 待完善/缺失部分
- [ ] **测试覆盖率**: `src/olav/tools/knowledge_embedder.py` 单元测试缺失 (目前 0%)。
- [ ] **物理删除同步**: 手动删除 `.olav/knowledge/` 下的文件目前无法自动从 DuckDB 中清除。
...
#### 5.7 缺失与待完善
- [ ] **物理文件删除同步**: `nornir_bulk_execute` 的并行执行在部分场景下可能因 Nornir 锁问题导致阻塞。
- [ ] **交互式 Plan**: 命令行中 `/analyze --plan` 目前仅显示静态文本，尚未实现完整的交互式确认流程。
- [ ] **报告模板引擎**: 文档提到的 `src/olav/tools/report.py`目前实际为 `report_formatter.py`，需要统一命名。
- [ ] **Skill 目录一致性**: 文档中引用的 `.olav/skills/inspection/` 目录尚未创建，目前 Skill 散落在 `.olav/skills/device-inspection/` 等目录。
...
#### ⚠️ 迫切任务
- [ ] **测试补齐**: 补全 `src/olav/tools/knowledge_embedder.py` 的测试用例 (补齐 0% 缺口)。
- [ ] **同步机制**: 实现知识库物理删除与 DuckDB 索引的联动删除。
...
- [ ] **NetBox/Zabbix 原生集成**: 建立 Skill 级的第三方数据同步策略。
- [ ] **PostgresSaver 持久化**: 迁移内存/JSON 记忆到生产级数据库。
- [ ] **多租户支持**: Skill 隔离与权限细分。
...
- [x] `/inspect <scope>` - 快速巡检 (Workflow)
- [x] `/backup [scope]` - 配置备份 (Workflow)
- [x] `/analyze <src> <dst>` - 路径分析 (Workflow)
- [x] `/reload` - 重载配置
- [x] `/clear` - 清除记忆
- [x] `/history` - 查看历史
- [x] `/help` - 帮助信息
