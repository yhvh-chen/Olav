...
**实施状态**: ❌ **未实施** - 硬编码路径仍存在，需在 v0.8.3 统一迁移至 settings。

检查中发现以下路径和参数通过源码"硬编码"，**计划在 v0.8.3** 迁移至 `config/settings.py` 统一管理：

### 需外移的硬编码配置
- **拓扑数据库路径**: ❌ **未配置**
  - 当前代码: `src/olav/tools/topology_importer.py` -> `".olav/db/network_warehouse.duckdb"`
  - 应改为: `settings.TOPOLOGY_DB_PATH` 或 `settings.NETWORK_WAREHOUSE_PATH`
  - 影响范围: 5 个文件 (topology_importer.py, topology_graph.py, database.py, sync_tools.py)
  
- **可视化输出目录**: ❌ **未配置**
- **可视化输出目录**: ❌ **未配置**
  - 当前代码: `src/olav/tools/topology_viz.py` -> `Path("exports/visualizations")`
  - 应改为: `settings.VISUALIZATION_DIR`
  - 影响范围: 2 个文件 (topology_viz.py, sync_tools.py)
  
- **默认超时**: ⚠️ **部分实施**
  - MapConfig 已定义超时配置 (map_scheduler.py: timeout_per_call=30.0)
  - 但未统一暴露到 settings.TIMEOUT_CONFIG
  - 部分 Nornir Task 超时仍为隐式默认值
  
- **日志分析窗口**: ❌ **未配置**
  - 当前代码: `src/olav/tools/sync_tools.py:686` -> `datetime.now() - timedelta(hours=24)`
  - 应改为: `settings.LOG_ANALYSIS_WINDOW_HOURS`
  
- **数据归档策略**: ❌ **未实现**
  - 目前缺失自动归档/清理逻辑，`exports/snapshots/sync/` 目录会无限增长
  - 需增加 `settings.RETENTION_DAYS` 配置和对应的清理工具
  
- **报告输出目录**: ✅ **已重构** (但未提取到 settings)
  - 目录结构已统一为 `exports/reports/{analysis,snapshots}/`
  - 但路径仍硬编码在工具代码中，未提取到 settings
  - 建议: 添加 `settings.EXPORTS_DIR`, `settings.REPORTS_ANALYSIS_DIR`, `settings.REPORTS_SNAPSHOTS_DIR`

### 当前 settings.py 状态
- ✅ 已配置: `duckdb_path` (registry.duckdb)
- ❌ 缺失: `TOPOLOGY_DB_PATH`, `VISUALIZATION_DIR`, `LOG_ANALYSIS_WINDOW_HOURS`, `RETENTION_DAYS`, `EXPORTS_DIR`
- ⚠️ 需更新: `knowledge_db_path` 从 `.olav/data/knowledge.db` 改为 `.olav/db/knowledge.duckdb`
...
- **数据归档 (Retention)**: ❌ **未实现** - `exports/snapshots/sync/` 会无限增长，无自动清理机制。
- **Cron 调度**: ❌ **未实现** - Workflow 中定义了 `schedule: "0 6 * * *"`，但无调度器实现（map_scheduler.py 仅为 Map-Reduce 执行控制）。
- **多语言报告**: ❌ **未实现** - `report_formatter.py:124` 中语言检测标记为 TODO。

### 功能验证状态
- ✅ **Knowledge Embedding**: 已实现 (`knowledge_embedder.py`, `learning_tools.py` 包含 `embed_knowledge_tool`)
  - 支持 Ollama (本地免费) 和 OpenAI (云端付费) 两种 Embedding Provider
  - 已集成到 storage_tools.py 用于自动 embedding 新增 Solution
- ⚠️ **Map-Reduce LLM 分析**: 已实现框架但未完全集成
  - `llm_interface.py` - MapReduceLLM 类已实现
  - `map_scheduler.py` - 异步并发控制已实现
  - `inspect-analyzer` 和 `log-analyzer` Skills 已定义
  - **待验证**: daily-run workflow Stage 3/4 的实际调用路径
- [ ] **Subagent 委派**: 需验证
  - `deep-analysis` Skill 中定义了 `macro-analyzer` 和 `micro-analyzer`
  - 需确认是否通过 DeepAgents 框架实际调用
- [ ] **LLM 流式输出**: `LLM_STREAMING_GUIDE.md` 存在，实际集成状态待确认
