...
## 7. 数据层与目录结构重构 (Proposed Refactoring - **未实施**)

> **⚠️ 重要说明**：本章内容为 **v0.8.2 计划中的重构**，v0.8.1 **未实施**。  
> **当前实际状态**：  
> - 目录结构：`data/sync/` (未改为 `exports/snapshots/`)  
> - Skill 名称：`daily-sync` (未改为 `network-snapshot`)  
> - CLI 命令：`/sync` (未改为 `/snapshot`)  

基于用户反馈，**计划在 v0.8.2** 对数据目录名称和结构进行语义化重构，以消除歧义并匹配实际功能。
...
**Week 1**: 目录与命名重构 (实施第 7 章提议)
- `data/sync/` → `exports/snapshots/`, `.olav/data/` → `.olav/db/`
- 数据库重命名 (`topology.db` → `network_warehouse.duckdb`)
- Skill 重命名 (`daily-sync` → `network-snapshot`)
- 命令重命名 (`/sync` → `/snapshot`)
