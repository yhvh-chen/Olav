...
from olav.workflows.batch_execution import BatchExecutionState, BatchExecutionWorkflow
from olav.workflows.device_execution import DeviceExecutionState, DeviceExecutionWorkflow
from olav.workflows.netbox_management import NetBoxManagementWorkflow
from olav.workflows.query_diagnostic import QueryDiagnosticState, QueryDiagnosticWorkflow
...
class TestQueryDiagnosticWorkflow:
    ...
    def test_workflow_properties(self, workflow):
        ...
        assert "query" in workflow.description.lower() or "diagnostic" in workflow.description.lower()
...
class TestDeviceExecutionWorkflow:
    ...
    async def test_validate_config_change_input(self, workflow):
        ...
        queries = [
            "modify BGP config",
            "add VLAN 100",
            "shutdown interface GigabitEthernet0/0/1",
...
class TestNetBoxManagementWorkflow:
    ...
    def test_workflow_properties(self, workflow):
        ...
        assert "NetBox" in workflow.description or "inventory" in workflow.description.lower()
...
class TestBatchExecutionWorkflow:
    ...
    async def test_validate_batch_input(self, workflow):
        ...
        queries = [
            ...
            "batch configure NTP servers",
            "set SNMP community on all devices",
            "modify MTU on all core routers",
    ...
    async def test_validate_rejects_single_device(self, workflow):
        ...
        queries = [
            "modify R1 BGP config",
            "configure Switch-A interface",
    ...
    def test_workflow_properties(self, workflow):
        ...
        assert "batch" in workflow.description.lower() or "multi-device" in workflow.description.lower()
        assert "netconf_tool" in workflow.tools_required
        assert "netbox_api_call" in workflow.tools_required
